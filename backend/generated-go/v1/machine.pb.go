// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.12
// 	protoc        (unknown)
// source: v1/machine.proto

package v1

import (
	_ "google.golang.org/genproto/googleapis/api/annotations"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	emptypb "google.golang.org/protobuf/types/known/emptypb"
	timestamppb "google.golang.org/protobuf/types/known/timestamppb"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// UploadEntryKind classifies one command-data entry. progress/event are the
// two existing per-(command) seq spaces (command_output / command_event dedup
// keys); result is the per-command terminal record.
type UploadEntryKind int32

const (
	UploadEntryKind_UPLOAD_ENTRY_KIND_UNSPECIFIED UploadEntryKind = 0
	UploadEntryKind_UPLOAD_ENTRY_KIND_PROGRESS    UploadEntryKind = 1
	UploadEntryKind_UPLOAD_ENTRY_KIND_EVENT       UploadEntryKind = 2
	UploadEntryKind_UPLOAD_ENTRY_KIND_RESULT      UploadEntryKind = 3
)

// Enum value maps for UploadEntryKind.
var (
	UploadEntryKind_name = map[int32]string{
		0: "UPLOAD_ENTRY_KIND_UNSPECIFIED",
		1: "UPLOAD_ENTRY_KIND_PROGRESS",
		2: "UPLOAD_ENTRY_KIND_EVENT",
		3: "UPLOAD_ENTRY_KIND_RESULT",
	}
	UploadEntryKind_value = map[string]int32{
		"UPLOAD_ENTRY_KIND_UNSPECIFIED": 0,
		"UPLOAD_ENTRY_KIND_PROGRESS":    1,
		"UPLOAD_ENTRY_KIND_EVENT":       2,
		"UPLOAD_ENTRY_KIND_RESULT":      3,
	}
)

func (x UploadEntryKind) Enum() *UploadEntryKind {
	p := new(UploadEntryKind)
	*p = x
	return p
}

func (x UploadEntryKind) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (UploadEntryKind) Descriptor() protoreflect.EnumDescriptor {
	return file_v1_machine_proto_enumTypes[0].Descriptor()
}

func (UploadEntryKind) Type() protoreflect.EnumType {
	return &file_v1_machine_proto_enumTypes[0]
}

func (x UploadEntryKind) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use UploadEntryKind.Descriptor instead.
func (UploadEntryKind) EnumDescriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{0}
}

// ProvisioningPhase is the API-level lifecycle of one provisioning job,
// mirroring laelia.store.ProvisioningPhase. The manager records transitions on
// Machine.provisioning; the provisioner drives them through
// ProvisionJobProgress frames.
type ProvisioningPhase int32

const (
	ProvisioningPhase_PROVISIONING_PHASE_UNSPECIFIED    ProvisioningPhase = 0
	ProvisioningPhase_PROVISIONING_PHASE_PENDING        ProvisioningPhase = 1
	ProvisioningPhase_PROVISIONING_PHASE_PROVISIONING   ProvisioningPhase = 2
	ProvisioningPhase_PROVISIONING_PHASE_PROVISIONED    ProvisioningPhase = 3
	ProvisioningPhase_PROVISIONING_PHASE_FAILED         ProvisioningPhase = 4
	ProvisioningPhase_PROVISIONING_PHASE_DEPROVISIONING ProvisioningPhase = 5
	ProvisioningPhase_PROVISIONING_PHASE_DELETED        ProvisioningPhase = 6
)

// Enum value maps for ProvisioningPhase.
var (
	ProvisioningPhase_name = map[int32]string{
		0: "PROVISIONING_PHASE_UNSPECIFIED",
		1: "PROVISIONING_PHASE_PENDING",
		2: "PROVISIONING_PHASE_PROVISIONING",
		3: "PROVISIONING_PHASE_PROVISIONED",
		4: "PROVISIONING_PHASE_FAILED",
		5: "PROVISIONING_PHASE_DEPROVISIONING",
		6: "PROVISIONING_PHASE_DELETED",
	}
	ProvisioningPhase_value = map[string]int32{
		"PROVISIONING_PHASE_UNSPECIFIED":    0,
		"PROVISIONING_PHASE_PENDING":        1,
		"PROVISIONING_PHASE_PROVISIONING":   2,
		"PROVISIONING_PHASE_PROVISIONED":    3,
		"PROVISIONING_PHASE_FAILED":         4,
		"PROVISIONING_PHASE_DEPROVISIONING": 5,
		"PROVISIONING_PHASE_DELETED":        6,
	}
)

func (x ProvisioningPhase) Enum() *ProvisioningPhase {
	p := new(ProvisioningPhase)
	*p = x
	return p
}

func (x ProvisioningPhase) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (ProvisioningPhase) Descriptor() protoreflect.EnumDescriptor {
	return file_v1_machine_proto_enumTypes[1].Descriptor()
}

func (ProvisioningPhase) Type() protoreflect.EnumType {
	return &file_v1_machine_proto_enumTypes[1]
}

func (x ProvisioningPhase) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use ProvisioningPhase.Descriptor instead.
func (ProvisioningPhase) EnumDescriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{1}
}

type MachineStatus_ConnectionState int32

const (
	MachineStatus_CONNECTION_STATE_UNSPECIFIED MachineStatus_ConnectionState = 0
	MachineStatus_ONLINE                       MachineStatus_ConnectionState = 1
	MachineStatus_OFFLINE                      MachineStatus_ConnectionState = 2
	MachineStatus_ERROR                        MachineStatus_ConnectionState = 3
	MachineStatus_KICKED                       MachineStatus_ConnectionState = 4
)

// Enum value maps for MachineStatus_ConnectionState.
var (
	MachineStatus_ConnectionState_name = map[int32]string{
		0: "CONNECTION_STATE_UNSPECIFIED",
		1: "ONLINE",
		2: "OFFLINE",
		3: "ERROR",
		4: "KICKED",
	}
	MachineStatus_ConnectionState_value = map[string]int32{
		"CONNECTION_STATE_UNSPECIFIED": 0,
		"ONLINE":                       1,
		"OFFLINE":                      2,
		"ERROR":                        3,
		"KICKED":                       4,
	}
)

func (x MachineStatus_ConnectionState) Enum() *MachineStatus_ConnectionState {
	p := new(MachineStatus_ConnectionState)
	*p = x
	return p
}

func (x MachineStatus_ConnectionState) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (MachineStatus_ConnectionState) Descriptor() protoreflect.EnumDescriptor {
	return file_v1_machine_proto_enumTypes[2].Descriptor()
}

func (MachineStatus_ConnectionState) Type() protoreflect.EnumType {
	return &file_v1_machine_proto_enumTypes[2]
}

func (x MachineStatus_ConnectionState) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use MachineStatus_ConnectionState.Descriptor instead.
func (MachineStatus_ConnectionState) EnumDescriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{38, 0}
}

// BeginSessionRequest names the agent whose drain loop is pulling work.
type BeginSessionRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// agent_name is the agent resource name (agents/{agent}); the manager
	// validates the authenticated machine owns this agent.
	AgentName     string `protobuf:"bytes,1,opt,name=agent_name,json=agentName,proto3" json:"agent_name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *BeginSessionRequest) Reset() {
	*x = BeginSessionRequest{}
	mi := &file_v1_machine_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *BeginSessionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BeginSessionRequest) ProtoMessage() {}

func (x *BeginSessionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BeginSessionRequest.ProtoReflect.Descriptor instead.
func (*BeginSessionRequest) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{0}
}

func (x *BeginSessionRequest) GetAgentName() string {
	if x != nil {
		return x.AgentName
	}
	return ""
}

// UploadCommandDataEntry is one command data record, mirroring the durable
// outbox envelope the machine appends per turn. seq_no is the per-(command,
// kind) sequence number the manager dedups on.
type UploadCommandDataEntry struct {
	state     protoimpl.MessageState `protogen:"open.v1"`
	CommandId string                 `protobuf:"bytes,1,opt,name=command_id,json=commandId,proto3" json:"command_id,omitempty"`
	Kind      UploadEntryKind        `protobuf:"varint,2,opt,name=kind,proto3,enum=laelia.v1.UploadEntryKind" json:"kind,omitempty"`
	SeqNo     int32                  `protobuf:"varint,3,opt,name=seq_no,json=seqNo,proto3" json:"seq_no,omitempty"`
	// agent_side_timestamp is the machine's wall-clock time when the record was
	// produced, so the manager can order and store it without adding arrival
	// delay. Empty means "use arrival time".
	AgentSideTimestamp *timestamppb.Timestamp `protobuf:"bytes,4,opt,name=agent_side_timestamp,json=agentSideTimestamp,proto3" json:"agent_side_timestamp,omitempty"`
	// Types that are valid to be assigned to Payload:
	//
	//	*UploadCommandDataEntry_Progress
	//	*UploadCommandDataEntry_Event
	//	*UploadCommandDataEntry_Result
	Payload       isUploadCommandDataEntry_Payload `protobuf_oneof:"payload"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UploadCommandDataEntry) Reset() {
	*x = UploadCommandDataEntry{}
	mi := &file_v1_machine_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UploadCommandDataEntry) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UploadCommandDataEntry) ProtoMessage() {}

func (x *UploadCommandDataEntry) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UploadCommandDataEntry.ProtoReflect.Descriptor instead.
func (*UploadCommandDataEntry) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{1}
}

func (x *UploadCommandDataEntry) GetCommandId() string {
	if x != nil {
		return x.CommandId
	}
	return ""
}

func (x *UploadCommandDataEntry) GetKind() UploadEntryKind {
	if x != nil {
		return x.Kind
	}
	return UploadEntryKind_UPLOAD_ENTRY_KIND_UNSPECIFIED
}

func (x *UploadCommandDataEntry) GetSeqNo() int32 {
	if x != nil {
		return x.SeqNo
	}
	return 0
}

func (x *UploadCommandDataEntry) GetAgentSideTimestamp() *timestamppb.Timestamp {
	if x != nil {
		return x.AgentSideTimestamp
	}
	return nil
}

func (x *UploadCommandDataEntry) GetPayload() isUploadCommandDataEntry_Payload {
	if x != nil {
		return x.Payload
	}
	return nil
}

func (x *UploadCommandDataEntry) GetProgress() *CommandProgress {
	if x != nil {
		if x, ok := x.Payload.(*UploadCommandDataEntry_Progress); ok {
			return x.Progress
		}
	}
	return nil
}

func (x *UploadCommandDataEntry) GetEvent() *CommandEvent {
	if x != nil {
		if x, ok := x.Payload.(*UploadCommandDataEntry_Event); ok {
			return x.Event
		}
	}
	return nil
}

func (x *UploadCommandDataEntry) GetResult() *CommandResult {
	if x != nil {
		if x, ok := x.Payload.(*UploadCommandDataEntry_Result); ok {
			return x.Result
		}
	}
	return nil
}

type isUploadCommandDataEntry_Payload interface {
	isUploadCommandDataEntry_Payload()
}

type UploadCommandDataEntry_Progress struct {
	Progress *CommandProgress `protobuf:"bytes,5,opt,name=progress,proto3,oneof"`
}

type UploadCommandDataEntry_Event struct {
	Event *CommandEvent `protobuf:"bytes,6,opt,name=event,proto3,oneof"`
}

type UploadCommandDataEntry_Result struct {
	Result *CommandResult `protobuf:"bytes,7,opt,name=result,proto3,oneof"`
}

func (*UploadCommandDataEntry_Progress) isUploadCommandDataEntry_Payload() {}

func (*UploadCommandDataEntry_Event) isUploadCommandDataEntry_Payload() {}

func (*UploadCommandDataEntry_Result) isUploadCommandDataEntry_Payload() {}

// UploadCommandDataRequest carries one uploader batch. Mixed commands are
// legal (forward-compatible with a machine-level uploader); within one
// command, entries are strictly seq-ascending per kind.
type UploadCommandDataRequest struct {
	state         protoimpl.MessageState    `protogen:"open.v1"`
	Entries       []*UploadCommandDataEntry `protobuf:"bytes,1,rep,name=entries,proto3" json:"entries,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UploadCommandDataRequest) Reset() {
	*x = UploadCommandDataRequest{}
	mi := &file_v1_machine_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UploadCommandDataRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UploadCommandDataRequest) ProtoMessage() {}

func (x *UploadCommandDataRequest) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UploadCommandDataRequest.ProtoReflect.Descriptor instead.
func (*UploadCommandDataRequest) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{2}
}

func (x *UploadCommandDataRequest) GetEntries() []*UploadCommandDataEntry {
	if x != nil {
		return x.Entries
	}
	return nil
}

// UploadCommandDataAck is the per-command persisted watermark of one batch.
// A kind's watermark is the highest seq_no of that kind persisted by the
// manager (contiguous from the machine's side because retransmission dedups).
type UploadCommandDataAck struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	CommandId       string                 `protobuf:"bytes,1,opt,name=command_id,json=commandId,proto3" json:"command_id,omitempty"`
	LastProgressSeq int32                  `protobuf:"varint,2,opt,name=last_progress_seq,json=lastProgressSeq,proto3" json:"last_progress_seq,omitempty"`
	LastEventSeq    int32                  `protobuf:"varint,3,opt,name=last_event_seq,json=lastEventSeq,proto3" json:"last_event_seq,omitempty"`
	// result_acked means the command's terminal record (and therefore every
	// record of the command in this batch and earlier) is durably persisted.
	ResultAcked   bool `protobuf:"varint,4,opt,name=result_acked,json=resultAcked,proto3" json:"result_acked,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UploadCommandDataAck) Reset() {
	*x = UploadCommandDataAck{}
	mi := &file_v1_machine_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UploadCommandDataAck) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UploadCommandDataAck) ProtoMessage() {}

func (x *UploadCommandDataAck) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UploadCommandDataAck.ProtoReflect.Descriptor instead.
func (*UploadCommandDataAck) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{3}
}

func (x *UploadCommandDataAck) GetCommandId() string {
	if x != nil {
		return x.CommandId
	}
	return ""
}

func (x *UploadCommandDataAck) GetLastProgressSeq() int32 {
	if x != nil {
		return x.LastProgressSeq
	}
	return 0
}

func (x *UploadCommandDataAck) GetLastEventSeq() int32 {
	if x != nil {
		return x.LastEventSeq
	}
	return 0
}

func (x *UploadCommandDataAck) GetResultAcked() bool {
	if x != nil {
		return x.ResultAcked
	}
	return false
}

// UploadCommandDataRejection explicitly lists one entry the manager refused.
// A watermark cannot express holes inside a batch, so rejected entries are
// enumerated and treated as "never retransmit" by the uploader.
type UploadCommandDataRejection struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	CommandId     string                 `protobuf:"bytes,1,opt,name=command_id,json=commandId,proto3" json:"command_id,omitempty"`
	Kind          UploadEntryKind        `protobuf:"varint,2,opt,name=kind,proto3,enum=laelia.v1.UploadEntryKind" json:"kind,omitempty"`
	SeqNo         int32                  `protobuf:"varint,3,opt,name=seq_no,json=seqNo,proto3" json:"seq_no,omitempty"`
	Reason        string                 `protobuf:"bytes,4,opt,name=reason,proto3" json:"reason,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UploadCommandDataRejection) Reset() {
	*x = UploadCommandDataRejection{}
	mi := &file_v1_machine_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UploadCommandDataRejection) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UploadCommandDataRejection) ProtoMessage() {}

func (x *UploadCommandDataRejection) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UploadCommandDataRejection.ProtoReflect.Descriptor instead.
func (*UploadCommandDataRejection) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{4}
}

func (x *UploadCommandDataRejection) GetCommandId() string {
	if x != nil {
		return x.CommandId
	}
	return ""
}

func (x *UploadCommandDataRejection) GetKind() UploadEntryKind {
	if x != nil {
		return x.Kind
	}
	return UploadEntryKind_UPLOAD_ENTRY_KIND_UNSPECIFIED
}

func (x *UploadCommandDataRejection) GetSeqNo() int32 {
	if x != nil {
		return x.SeqNo
	}
	return 0
}

func (x *UploadCommandDataRejection) GetReason() string {
	if x != nil {
		return x.Reason
	}
	return ""
}

type UploadCommandDataResponse struct {
	state         protoimpl.MessageState        `protogen:"open.v1"`
	Acks          []*UploadCommandDataAck       `protobuf:"bytes,1,rep,name=acks,proto3" json:"acks,omitempty"`
	Rejected      []*UploadCommandDataRejection `protobuf:"bytes,2,rep,name=rejected,proto3" json:"rejected,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UploadCommandDataResponse) Reset() {
	*x = UploadCommandDataResponse{}
	mi := &file_v1_machine_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UploadCommandDataResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UploadCommandDataResponse) ProtoMessage() {}

func (x *UploadCommandDataResponse) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UploadCommandDataResponse.ProtoReflect.Descriptor instead.
func (*UploadCommandDataResponse) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{5}
}

func (x *UploadCommandDataResponse) GetAcks() []*UploadCommandDataAck {
	if x != nil {
		return x.Acks
	}
	return nil
}

func (x *UploadCommandDataResponse) GetRejected() []*UploadCommandDataRejection {
	if x != nil {
		return x.Rejected
	}
	return nil
}

type UpdateMachineRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	Name  string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// New machine title. Empty means "leave unchanged".
	Title         string `protobuf:"bytes,2,opt,name=title,proto3" json:"title,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateMachineRequest) Reset() {
	*x = UpdateMachineRequest{}
	mi := &file_v1_machine_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateMachineRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateMachineRequest) ProtoMessage() {}

func (x *UpdateMachineRequest) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateMachineRequest.ProtoReflect.Descriptor instead.
func (*UpdateMachineRequest) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{6}
}

func (x *UpdateMachineRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *UpdateMachineRequest) GetTitle() string {
	if x != nil {
		return x.Title
	}
	return ""
}

type TransferMachineOwnershipRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	Name  string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// New owner's user resource name (users/{id}).
	NewOwner      string `protobuf:"bytes,2,opt,name=new_owner,json=newOwner,proto3" json:"new_owner,omitempty"`
	Reason        string `protobuf:"bytes,3,opt,name=reason,proto3" json:"reason,omitempty"` // audit purpose
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *TransferMachineOwnershipRequest) Reset() {
	*x = TransferMachineOwnershipRequest{}
	mi := &file_v1_machine_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TransferMachineOwnershipRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TransferMachineOwnershipRequest) ProtoMessage() {}

func (x *TransferMachineOwnershipRequest) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TransferMachineOwnershipRequest.ProtoReflect.Descriptor instead.
func (*TransferMachineOwnershipRequest) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{7}
}

func (x *TransferMachineOwnershipRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *TransferMachineOwnershipRequest) GetNewOwner() string {
	if x != nil {
		return x.NewOwner
	}
	return ""
}

func (x *TransferMachineOwnershipRequest) GetReason() string {
	if x != nil {
		return x.Reason
	}
	return ""
}

type TransferMachineOwnershipResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Machine       *Machine               `protobuf:"bytes,1,opt,name=machine,proto3" json:"machine,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *TransferMachineOwnershipResponse) Reset() {
	*x = TransferMachineOwnershipResponse{}
	mi := &file_v1_machine_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TransferMachineOwnershipResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TransferMachineOwnershipResponse) ProtoMessage() {}

func (x *TransferMachineOwnershipResponse) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TransferMachineOwnershipResponse.ProtoReflect.Descriptor instead.
func (*TransferMachineOwnershipResponse) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{8}
}

func (x *TransferMachineOwnershipResponse) GetMachine() *Machine {
	if x != nil {
		return x.Machine
	}
	return nil
}

type RevokeMachineTokenRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Name          string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	Reason        string                 `protobuf:"bytes,2,opt,name=reason,proto3" json:"reason,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RevokeMachineTokenRequest) Reset() {
	*x = RevokeMachineTokenRequest{}
	mi := &file_v1_machine_proto_msgTypes[9]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RevokeMachineTokenRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RevokeMachineTokenRequest) ProtoMessage() {}

func (x *RevokeMachineTokenRequest) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[9]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RevokeMachineTokenRequest.ProtoReflect.Descriptor instead.
func (*RevokeMachineTokenRequest) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{9}
}

func (x *RevokeMachineTokenRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *RevokeMachineTokenRequest) GetReason() string {
	if x != nil {
		return x.Reason
	}
	return ""
}

type RevokeMachineTokenResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RevokeMachineTokenResponse) Reset() {
	*x = RevokeMachineTokenResponse{}
	mi := &file_v1_machine_proto_msgTypes[10]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RevokeMachineTokenResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RevokeMachineTokenResponse) ProtoMessage() {}

func (x *RevokeMachineTokenResponse) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[10]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RevokeMachineTokenResponse.ProtoReflect.Descriptor instead.
func (*RevokeMachineTokenResponse) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{10}
}

type ForceDisconnectMachineRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Name          string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	Reason        string                 `protobuf:"bytes,2,opt,name=reason,proto3" json:"reason,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ForceDisconnectMachineRequest) Reset() {
	*x = ForceDisconnectMachineRequest{}
	mi := &file_v1_machine_proto_msgTypes[11]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ForceDisconnectMachineRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ForceDisconnectMachineRequest) ProtoMessage() {}

func (x *ForceDisconnectMachineRequest) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[11]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ForceDisconnectMachineRequest.ProtoReflect.Descriptor instead.
func (*ForceDisconnectMachineRequest) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{11}
}

func (x *ForceDisconnectMachineRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *ForceDisconnectMachineRequest) GetReason() string {
	if x != nil {
		return x.Reason
	}
	return ""
}

type ListMachineAgentsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Name          string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	PageSize      int32                  `protobuf:"varint,2,opt,name=page_size,json=pageSize,proto3" json:"page_size,omitempty"`
	PageToken     string                 `protobuf:"bytes,3,opt,name=page_token,json=pageToken,proto3" json:"page_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListMachineAgentsRequest) Reset() {
	*x = ListMachineAgentsRequest{}
	mi := &file_v1_machine_proto_msgTypes[12]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListMachineAgentsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListMachineAgentsRequest) ProtoMessage() {}

func (x *ListMachineAgentsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[12]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListMachineAgentsRequest.ProtoReflect.Descriptor instead.
func (*ListMachineAgentsRequest) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{12}
}

func (x *ListMachineAgentsRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *ListMachineAgentsRequest) GetPageSize() int32 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

func (x *ListMachineAgentsRequest) GetPageToken() string {
	if x != nil {
		return x.PageToken
	}
	return ""
}

type ListMachineAgentsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Agents        []*AgentSummary        `protobuf:"bytes,1,rep,name=agents,proto3" json:"agents,omitempty"`
	NextPageToken string                 `protobuf:"bytes,2,opt,name=next_page_token,json=nextPageToken,proto3" json:"next_page_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListMachineAgentsResponse) Reset() {
	*x = ListMachineAgentsResponse{}
	mi := &file_v1_machine_proto_msgTypes[13]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListMachineAgentsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListMachineAgentsResponse) ProtoMessage() {}

func (x *ListMachineAgentsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[13]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListMachineAgentsResponse.ProtoReflect.Descriptor instead.
func (*ListMachineAgentsResponse) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{13}
}

func (x *ListMachineAgentsResponse) GetAgents() []*AgentSummary {
	if x != nil {
		return x.Agents
	}
	return nil
}

func (x *ListMachineAgentsResponse) GetNextPageToken() string {
	if x != nil {
		return x.NextPageToken
	}
	return ""
}

type RefreshMachineProvidersRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Name          string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RefreshMachineProvidersRequest) Reset() {
	*x = RefreshMachineProvidersRequest{}
	mi := &file_v1_machine_proto_msgTypes[14]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RefreshMachineProvidersRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RefreshMachineProvidersRequest) ProtoMessage() {}

func (x *RefreshMachineProvidersRequest) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[14]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RefreshMachineProvidersRequest.ProtoReflect.Descriptor instead.
func (*RefreshMachineProvidersRequest) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{14}
}

func (x *RefreshMachineProvidersRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

type RefreshMachineProvidersResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Providers     []*AgentProviderInfo   `protobuf:"bytes,1,rep,name=providers,proto3" json:"providers,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RefreshMachineProvidersResponse) Reset() {
	*x = RefreshMachineProvidersResponse{}
	mi := &file_v1_machine_proto_msgTypes[15]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RefreshMachineProvidersResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RefreshMachineProvidersResponse) ProtoMessage() {}

func (x *RefreshMachineProvidersResponse) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[15]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RefreshMachineProvidersResponse.ProtoReflect.Descriptor instead.
func (*RefreshMachineProvidersResponse) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{15}
}

func (x *RefreshMachineProvidersResponse) GetProviders() []*AgentProviderInfo {
	if x != nil {
		return x.Providers
	}
	return nil
}

// RefreshMachineModelsRequest names a machine and carries the (possibly
// unsaved) ACP config whose custom_env the probe should apply. The provider to
// probe and the env overlay are taken from acp_config; everything else ignored.
type RefreshMachineModelsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Name          string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	AcpConfig     *AgentACPConfig        `protobuf:"bytes,2,opt,name=acp_config,json=acpConfig,proto3" json:"acp_config,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RefreshMachineModelsRequest) Reset() {
	*x = RefreshMachineModelsRequest{}
	mi := &file_v1_machine_proto_msgTypes[16]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RefreshMachineModelsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RefreshMachineModelsRequest) ProtoMessage() {}

func (x *RefreshMachineModelsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[16]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RefreshMachineModelsRequest.ProtoReflect.Descriptor instead.
func (*RefreshMachineModelsRequest) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{16}
}

func (x *RefreshMachineModelsRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *RefreshMachineModelsRequest) GetAcpConfig() *AgentACPConfig {
	if x != nil {
		return x.AcpConfig
	}
	return nil
}

type RefreshMachineModelsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Provider      string                 `protobuf:"bytes,1,opt,name=provider,proto3" json:"provider,omitempty"`
	Models        []*AgentModelOption    `protobuf:"bytes,2,rep,name=models,proto3" json:"models,omitempty"` // freshly probed models for the provider
	Error         string                 `protobuf:"bytes,3,opt,name=error,proto3" json:"error,omitempty"`   // probe failure message; empty on success
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RefreshMachineModelsResponse) Reset() {
	*x = RefreshMachineModelsResponse{}
	mi := &file_v1_machine_proto_msgTypes[17]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RefreshMachineModelsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RefreshMachineModelsResponse) ProtoMessage() {}

func (x *RefreshMachineModelsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[17]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RefreshMachineModelsResponse.ProtoReflect.Descriptor instead.
func (*RefreshMachineModelsResponse) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{17}
}

func (x *RefreshMachineModelsResponse) GetProvider() string {
	if x != nil {
		return x.Provider
	}
	return ""
}

func (x *RefreshMachineModelsResponse) GetModels() []*AgentModelOption {
	if x != nil {
		return x.Models
	}
	return nil
}

func (x *RefreshMachineModelsResponse) GetError() string {
	if x != nil {
		return x.Error
	}
	return ""
}

type UpgradeMachineRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Name          string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	Reason        string                 `protobuf:"bytes,2,opt,name=reason,proto3" json:"reason,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpgradeMachineRequest) Reset() {
	*x = UpgradeMachineRequest{}
	mi := &file_v1_machine_proto_msgTypes[18]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpgradeMachineRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpgradeMachineRequest) ProtoMessage() {}

func (x *UpgradeMachineRequest) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[18]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpgradeMachineRequest.ProtoReflect.Descriptor instead.
func (*UpgradeMachineRequest) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{18}
}

func (x *UpgradeMachineRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *UpgradeMachineRequest) GetReason() string {
	if x != nil {
		return x.Reason
	}
	return ""
}

type ListMachineWorkspacesRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Name          string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListMachineWorkspacesRequest) Reset() {
	*x = ListMachineWorkspacesRequest{}
	mi := &file_v1_machine_proto_msgTypes[19]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListMachineWorkspacesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListMachineWorkspacesRequest) ProtoMessage() {}

func (x *ListMachineWorkspacesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[19]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListMachineWorkspacesRequest.ProtoReflect.Descriptor instead.
func (*ListMachineWorkspacesRequest) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{19}
}

func (x *ListMachineWorkspacesRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

type ListMachineWorkspacesResponse struct {
	state         protoimpl.MessageState     `protogen:"open.v1"`
	Workspaces    []*MachineWorkspaceSummary `protobuf:"bytes,1,rep,name=workspaces,proto3" json:"workspaces,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListMachineWorkspacesResponse) Reset() {
	*x = ListMachineWorkspacesResponse{}
	mi := &file_v1_machine_proto_msgTypes[20]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListMachineWorkspacesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListMachineWorkspacesResponse) ProtoMessage() {}

func (x *ListMachineWorkspacesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[20]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListMachineWorkspacesResponse.ProtoReflect.Descriptor instead.
func (*ListMachineWorkspacesResponse) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{20}
}

func (x *ListMachineWorkspacesResponse) GetWorkspaces() []*MachineWorkspaceSummary {
	if x != nil {
		return x.Workspaces
	}
	return nil
}

type ConnectMachineRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Info          *MachineInfo           `protobuf:"bytes,1,opt,name=info,proto3" json:"info,omitempty"`
	Fingerprint   string                 `protobuf:"bytes,2,opt,name=fingerprint,proto3" json:"fingerprint,omitempty"` // client-generated connection fingerprint (hostname:os:arch)
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ConnectMachineRequest) Reset() {
	*x = ConnectMachineRequest{}
	mi := &file_v1_machine_proto_msgTypes[21]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ConnectMachineRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ConnectMachineRequest) ProtoMessage() {}

func (x *ConnectMachineRequest) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[21]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ConnectMachineRequest.ProtoReflect.Descriptor instead.
func (*ConnectMachineRequest) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{21}
}

func (x *ConnectMachineRequest) GetInfo() *MachineInfo {
	if x != nil {
		return x.Info
	}
	return nil
}

func (x *ConnectMachineRequest) GetFingerprint() string {
	if x != nil {
		return x.Fingerprint
	}
	return ""
}

type ConnectMachineResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	SessionId     string                 `protobuf:"bytes,1,opt,name=session_id,json=sessionId,proto3" json:"session_id,omitempty"`
	InitialStatus *MachineStatus         `protobuf:"bytes,2,opt,name=initial_status,json=initialStatus,proto3" json:"initial_status,omitempty"`
	// The full set of agents this machine must host. The machine app starts a
	// runner per entry immediately after connect (and on every reconnect).
	AssignedAgents []*AgentAssignment `protobuf:"bytes,3,rep,name=assigned_agents,json=assignedAgents,proto3" json:"assigned_agents,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *ConnectMachineResponse) Reset() {
	*x = ConnectMachineResponse{}
	mi := &file_v1_machine_proto_msgTypes[22]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ConnectMachineResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ConnectMachineResponse) ProtoMessage() {}

func (x *ConnectMachineResponse) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[22]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ConnectMachineResponse.ProtoReflect.Descriptor instead.
func (*ConnectMachineResponse) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{22}
}

func (x *ConnectMachineResponse) GetSessionId() string {
	if x != nil {
		return x.SessionId
	}
	return ""
}

func (x *ConnectMachineResponse) GetInitialStatus() *MachineStatus {
	if x != nil {
		return x.InitialStatus
	}
	return nil
}

func (x *ConnectMachineResponse) GetAssignedAgents() []*AgentAssignment {
	if x != nil {
		return x.AssignedAgents
	}
	return nil
}

type MachineHeartbeatRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	SessionId     string                 `protobuf:"bytes,1,opt,name=session_id,json=sessionId,proto3" json:"session_id,omitempty"`
	PreviousNonce string                 `protobuf:"bytes,2,opt,name=previous_nonce,json=previousNonce,proto3" json:"previous_nonce,omitempty"` // nonce from previous response (replay protection)
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MachineHeartbeatRequest) Reset() {
	*x = MachineHeartbeatRequest{}
	mi := &file_v1_machine_proto_msgTypes[23]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MachineHeartbeatRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MachineHeartbeatRequest) ProtoMessage() {}

func (x *MachineHeartbeatRequest) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[23]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MachineHeartbeatRequest.ProtoReflect.Descriptor instead.
func (*MachineHeartbeatRequest) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{23}
}

func (x *MachineHeartbeatRequest) GetSessionId() string {
	if x != nil {
		return x.SessionId
	}
	return ""
}

func (x *MachineHeartbeatRequest) GetPreviousNonce() string {
	if x != nil {
		return x.PreviousNonce
	}
	return ""
}

type MachineHeartbeatResponse struct {
	state                protoimpl.MessageState `protogen:"open.v1"`
	NextNonce            string                 `protobuf:"bytes,1,opt,name=next_nonce,json=nextNonce,proto3" json:"next_nonce,omitempty"`
	NextHeartbeatAt      *timestamppb.Timestamp `protobuf:"bytes,2,opt,name=next_heartbeat_at,json=nextHeartbeatAt,proto3" json:"next_heartbeat_at,omitempty"`
	AccessToken          string                 `protobuf:"bytes,3,opt,name=access_token,json=accessToken,proto3" json:"access_token,omitempty"` // new access token (only if expiring soon)
	AccessTokenExpiresAt *timestamppb.Timestamp `protobuf:"bytes,4,opt,name=access_token_expires_at,json=accessTokenExpiresAt,proto3" json:"access_token_expires_at,omitempty"`
	unknownFields        protoimpl.UnknownFields
	sizeCache            protoimpl.SizeCache
}

func (x *MachineHeartbeatResponse) Reset() {
	*x = MachineHeartbeatResponse{}
	mi := &file_v1_machine_proto_msgTypes[24]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MachineHeartbeatResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MachineHeartbeatResponse) ProtoMessage() {}

func (x *MachineHeartbeatResponse) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[24]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MachineHeartbeatResponse.ProtoReflect.Descriptor instead.
func (*MachineHeartbeatResponse) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{24}
}

func (x *MachineHeartbeatResponse) GetNextNonce() string {
	if x != nil {
		return x.NextNonce
	}
	return ""
}

func (x *MachineHeartbeatResponse) GetNextHeartbeatAt() *timestamppb.Timestamp {
	if x != nil {
		return x.NextHeartbeatAt
	}
	return nil
}

func (x *MachineHeartbeatResponse) GetAccessToken() string {
	if x != nil {
		return x.AccessToken
	}
	return ""
}

func (x *MachineHeartbeatResponse) GetAccessTokenExpiresAt() *timestamppb.Timestamp {
	if x != nil {
		return x.AccessTokenExpiresAt
	}
	return nil
}

type MachineDisconnectRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	SessionId     string                 `protobuf:"bytes,1,opt,name=session_id,json=sessionId,proto3" json:"session_id,omitempty"`
	Reason        string                 `protobuf:"bytes,2,opt,name=reason,proto3" json:"reason,omitempty"` // "shutdown", "upgrade" etc.
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MachineDisconnectRequest) Reset() {
	*x = MachineDisconnectRequest{}
	mi := &file_v1_machine_proto_msgTypes[25]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MachineDisconnectRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MachineDisconnectRequest) ProtoMessage() {}

func (x *MachineDisconnectRequest) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[25]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MachineDisconnectRequest.ProtoReflect.Descriptor instead.
func (*MachineDisconnectRequest) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{25}
}

func (x *MachineDisconnectRequest) GetSessionId() string {
	if x != nil {
		return x.SessionId
	}
	return ""
}

func (x *MachineDisconnectRequest) GetReason() string {
	if x != nil {
		return x.Reason
	}
	return ""
}

type RefreshMachineTokenRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	RefreshToken  string                 `protobuf:"bytes,1,opt,name=refresh_token,json=refreshToken,proto3" json:"refresh_token,omitempty"`
	Fingerprint   string                 `protobuf:"bytes,2,opt,name=fingerprint,proto3" json:"fingerprint,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RefreshMachineTokenRequest) Reset() {
	*x = RefreshMachineTokenRequest{}
	mi := &file_v1_machine_proto_msgTypes[26]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RefreshMachineTokenRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RefreshMachineTokenRequest) ProtoMessage() {}

func (x *RefreshMachineTokenRequest) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[26]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RefreshMachineTokenRequest.ProtoReflect.Descriptor instead.
func (*RefreshMachineTokenRequest) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{26}
}

func (x *RefreshMachineTokenRequest) GetRefreshToken() string {
	if x != nil {
		return x.RefreshToken
	}
	return ""
}

func (x *RefreshMachineTokenRequest) GetFingerprint() string {
	if x != nil {
		return x.Fingerprint
	}
	return ""
}

type RefreshMachineTokenResponse struct {
	state                protoimpl.MessageState `protogen:"open.v1"`
	AccessToken          string                 `protobuf:"bytes,1,opt,name=access_token,json=accessToken,proto3" json:"access_token,omitempty"`
	RefreshToken         string                 `protobuf:"bytes,2,opt,name=refresh_token,json=refreshToken,proto3" json:"refresh_token,omitempty"` // new refresh token (rotation)
	AccessTokenExpiresAt *timestamppb.Timestamp `protobuf:"bytes,3,opt,name=access_token_expires_at,json=accessTokenExpiresAt,proto3" json:"access_token_expires_at,omitempty"`
	unknownFields        protoimpl.UnknownFields
	sizeCache            protoimpl.SizeCache
}

func (x *RefreshMachineTokenResponse) Reset() {
	*x = RefreshMachineTokenResponse{}
	mi := &file_v1_machine_proto_msgTypes[27]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RefreshMachineTokenResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RefreshMachineTokenResponse) ProtoMessage() {}

func (x *RefreshMachineTokenResponse) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[27]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RefreshMachineTokenResponse.ProtoReflect.Descriptor instead.
func (*RefreshMachineTokenResponse) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{27}
}

func (x *RefreshMachineTokenResponse) GetAccessToken() string {
	if x != nil {
		return x.AccessToken
	}
	return ""
}

func (x *RefreshMachineTokenResponse) GetRefreshToken() string {
	if x != nil {
		return x.RefreshToken
	}
	return ""
}

func (x *RefreshMachineTokenResponse) GetAccessTokenExpiresAt() *timestamppb.Timestamp {
	if x != nil {
		return x.AccessTokenExpiresAt
	}
	return nil
}

type ListMachinesRequest struct {
	state       protoimpl.MessageState `protogen:"open.v1"`
	PageSize    int32                  `protobuf:"varint,1,opt,name=page_size,json=pageSize,proto3" json:"page_size,omitempty"`
	PageToken   string                 `protobuf:"bytes,2,opt,name=page_token,json=pageToken,proto3" json:"page_token,omitempty"`
	ShowDeleted bool                   `protobuf:"varint,3,opt,name=show_deleted,json=showDeleted,proto3" json:"show_deleted,omitempty"`
	// Restrict the listing to machines created by the given provisioner
	// (provisioners/{id}). Empty lists every machine the caller may see.
	Provisioner   string `protobuf:"bytes,4,opt,name=provisioner,proto3" json:"provisioner,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListMachinesRequest) Reset() {
	*x = ListMachinesRequest{}
	mi := &file_v1_machine_proto_msgTypes[28]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListMachinesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListMachinesRequest) ProtoMessage() {}

func (x *ListMachinesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[28]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListMachinesRequest.ProtoReflect.Descriptor instead.
func (*ListMachinesRequest) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{28}
}

func (x *ListMachinesRequest) GetPageSize() int32 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

func (x *ListMachinesRequest) GetPageToken() string {
	if x != nil {
		return x.PageToken
	}
	return ""
}

func (x *ListMachinesRequest) GetShowDeleted() bool {
	if x != nil {
		return x.ShowDeleted
	}
	return false
}

func (x *ListMachinesRequest) GetProvisioner() string {
	if x != nil {
		return x.Provisioner
	}
	return ""
}

type ListMachinesResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Machines      []*MachineSummary      `protobuf:"bytes,1,rep,name=machines,proto3" json:"machines,omitempty"`
	NextPageToken string                 `protobuf:"bytes,2,opt,name=next_page_token,json=nextPageToken,proto3" json:"next_page_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListMachinesResponse) Reset() {
	*x = ListMachinesResponse{}
	mi := &file_v1_machine_proto_msgTypes[29]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListMachinesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListMachinesResponse) ProtoMessage() {}

func (x *ListMachinesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[29]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListMachinesResponse.ProtoReflect.Descriptor instead.
func (*ListMachinesResponse) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{29}
}

func (x *ListMachinesResponse) GetMachines() []*MachineSummary {
	if x != nil {
		return x.Machines
	}
	return nil
}

func (x *ListMachinesResponse) GetNextPageToken() string {
	if x != nil {
		return x.NextPageToken
	}
	return ""
}

type GetMachineRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Name          string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetMachineRequest) Reset() {
	*x = GetMachineRequest{}
	mi := &file_v1_machine_proto_msgTypes[30]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetMachineRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetMachineRequest) ProtoMessage() {}

func (x *GetMachineRequest) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[30]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetMachineRequest.ProtoReflect.Descriptor instead.
func (*GetMachineRequest) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{30}
}

func (x *GetMachineRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

type DeleteMachineRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Name          string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteMachineRequest) Reset() {
	*x = DeleteMachineRequest{}
	mi := &file_v1_machine_proto_msgTypes[31]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteMachineRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteMachineRequest) ProtoMessage() {}

func (x *DeleteMachineRequest) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[31]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteMachineRequest.ProtoReflect.Descriptor instead.
func (*DeleteMachineRequest) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{31}
}

func (x *DeleteMachineRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

type Machine struct {
	state     protoimpl.MessageState `protogen:"open.v1"`
	Name      string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	State     State                  `protobuf:"varint,2,opt,name=state,proto3,enum=laelia.v1.State" json:"state,omitempty"`
	Title     string                 `protobuf:"bytes,3,opt,name=title,proto3" json:"title,omitempty"`
	Info      *MachineInfo           `protobuf:"bytes,5,opt,name=info,proto3" json:"info,omitempty"`
	Status    *MachineStatus         `protobuf:"bytes,6,opt,name=status,proto3" json:"status,omitempty"`
	CreatedAt *timestamppb.Timestamp `protobuf:"bytes,7,opt,name=created_at,json=createdAt,proto3" json:"created_at,omitempty"`
	Labels    map[string]string      `protobuf:"bytes,8,rep,name=labels,proto3" json:"labels,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	// Creator's user resource name (users/{id}).
	CreatedBy string `protobuf:"bytes,11,opt,name=created_by,json=createdBy,proto3" json:"created_by,omitempty"`
	// can_edit reports whether the current caller holds laelia.machines.edit
	// (workspace-scope).
	CanEdit bool `protobuf:"varint,12,opt,name=can_edit,json=canEdit,proto3" json:"can_edit,omitempty"`
	// can_create_agent reports whether the current caller may create agents on
	// this machine: the machine's creator, a workspace admin, or a principal
	// bound to roles/machineAgentCreator in the machine's IAM policy.
	CanCreateAgent bool `protobuf:"varint,13,opt,name=can_create_agent,json=canCreateAgent,proto3" json:"can_create_agent,omitempty"`
	// can_manage reports whether the current caller may manage this machine's
	// IAM policy (the machine's creator or a workspace admin).
	CanManage bool `protobuf:"varint,14,opt,name=can_manage,json=canManage,proto3" json:"can_manage,omitempty"`
	// latest_version is the machine binary version embedded in this manager
	// (from the build manifest). Empty when the manager has no embedded
	// binaries, in which case upgrades are not offered.
	LatestVersion string `protobuf:"bytes,15,opt,name=latest_version,json=latestVersion,proto3" json:"latest_version,omitempty"`
	// upgrade_available reports whether the connected machine's reported version
	// is older than latest_version.
	UpgradeAvailable bool `protobuf:"varint,16,opt,name=upgrade_available,json=upgradeAvailable,proto3" json:"upgrade_available,omitempty"`
	// upgrade_status is the live progress of an in-flight (or last completed)
	// self-upgrade triggered via UpgradeMachine.
	UpgradeStatus *UpgradeProgress `protobuf:"bytes,17,opt,name=upgrade_status,json=upgradeStatus,proto3" json:"upgrade_status,omitempty"`
	// provisioning is the lifecycle state of this machine's provisioning job.
	// Set only for machines created through a provisioner; self-hosted machines
	// leave it unset.
	Provisioning *ProvisioningStatus `protobuf:"bytes,18,opt,name=provisioning,proto3" json:"provisioning,omitempty"`
	// provisioner is the resource name of the provisioner that created (and
	// manages the workload of) this machine, provisioners/{id}. Empty for
	// self-hosted machines.
	Provisioner   string `protobuf:"bytes,19,opt,name=provisioner,proto3" json:"provisioner,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Machine) Reset() {
	*x = Machine{}
	mi := &file_v1_machine_proto_msgTypes[32]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Machine) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Machine) ProtoMessage() {}

func (x *Machine) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[32]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Machine.ProtoReflect.Descriptor instead.
func (*Machine) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{32}
}

func (x *Machine) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *Machine) GetState() State {
	if x != nil {
		return x.State
	}
	return State_STATE_UNSPECIFIED
}

func (x *Machine) GetTitle() string {
	if x != nil {
		return x.Title
	}
	return ""
}

func (x *Machine) GetInfo() *MachineInfo {
	if x != nil {
		return x.Info
	}
	return nil
}

func (x *Machine) GetStatus() *MachineStatus {
	if x != nil {
		return x.Status
	}
	return nil
}

func (x *Machine) GetCreatedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.CreatedAt
	}
	return nil
}

func (x *Machine) GetLabels() map[string]string {
	if x != nil {
		return x.Labels
	}
	return nil
}

func (x *Machine) GetCreatedBy() string {
	if x != nil {
		return x.CreatedBy
	}
	return ""
}

func (x *Machine) GetCanEdit() bool {
	if x != nil {
		return x.CanEdit
	}
	return false
}

func (x *Machine) GetCanCreateAgent() bool {
	if x != nil {
		return x.CanCreateAgent
	}
	return false
}

func (x *Machine) GetCanManage() bool {
	if x != nil {
		return x.CanManage
	}
	return false
}

func (x *Machine) GetLatestVersion() string {
	if x != nil {
		return x.LatestVersion
	}
	return ""
}

func (x *Machine) GetUpgradeAvailable() bool {
	if x != nil {
		return x.UpgradeAvailable
	}
	return false
}

func (x *Machine) GetUpgradeStatus() *UpgradeProgress {
	if x != nil {
		return x.UpgradeStatus
	}
	return nil
}

func (x *Machine) GetProvisioning() *ProvisioningStatus {
	if x != nil {
		return x.Provisioning
	}
	return nil
}

func (x *Machine) GetProvisioner() string {
	if x != nil {
		return x.Provisioner
	}
	return ""
}

// MachineSummary is the lightweight list-view projection of a Machine returned
// by ListMachines. It carries identity, lifecycle state, connection status, and
// the count of agents bound to the machine.
type MachineSummary struct {
	state      protoimpl.MessageState `protogen:"open.v1"`
	Name       string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	State      State                  `protobuf:"varint,2,opt,name=state,proto3,enum=laelia.v1.State" json:"state,omitempty"`
	Title      string                 `protobuf:"bytes,3,opt,name=title,proto3" json:"title,omitempty"`
	Status     *MachineStatus         `protobuf:"bytes,4,opt,name=status,proto3" json:"status,omitempty"`
	AgentCount int32                  `protobuf:"varint,5,opt,name=agent_count,json=agentCount,proto3" json:"agent_count,omitempty"`
	// Creator's user resource name (users/{id}).
	CreatedBy string                 `protobuf:"bytes,6,opt,name=created_by,json=createdBy,proto3" json:"created_by,omitempty"`
	CreatedAt *timestamppb.Timestamp `protobuf:"bytes,10,opt,name=created_at,json=createdAt,proto3" json:"created_at,omitempty"`
	// can_edit reports whether the current caller holds laelia.machines.edit
	// (workspace-scope).
	CanEdit bool `protobuf:"varint,7,opt,name=can_edit,json=canEdit,proto3" json:"can_edit,omitempty"`
	// can_manage reports whether the current caller may manage this machine's
	// IAM policy (the machine's creator or a workspace admin).
	CanManage bool `protobuf:"varint,8,opt,name=can_manage,json=canManage,proto3" json:"can_manage,omitempty"`
	// can_delete reports whether the current caller may delete this machine:
	// the machine's creator or a holder of laelia.machines.delete.
	CanDelete bool `protobuf:"varint,9,opt,name=can_delete,json=canDelete,proto3" json:"can_delete,omitempty"`
	// latest_version / upgrade_available mirror Machine.latest_version and
	// Machine.upgrade_available for the list view's upgrade badge.
	LatestVersion    string `protobuf:"bytes,11,opt,name=latest_version,json=latestVersion,proto3" json:"latest_version,omitempty"`
	UpgradeAvailable bool   `protobuf:"varint,12,opt,name=upgrade_available,json=upgradeAvailable,proto3" json:"upgrade_available,omitempty"`
	// provisioning mirrors Machine.provisioning for the list view's provisioning
	// phase badge. Set only for provisioned machines.
	Provisioning  *ProvisioningStatus `protobuf:"bytes,13,opt,name=provisioning,proto3" json:"provisioning,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MachineSummary) Reset() {
	*x = MachineSummary{}
	mi := &file_v1_machine_proto_msgTypes[33]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MachineSummary) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MachineSummary) ProtoMessage() {}

func (x *MachineSummary) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[33]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MachineSummary.ProtoReflect.Descriptor instead.
func (*MachineSummary) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{33}
}

func (x *MachineSummary) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *MachineSummary) GetState() State {
	if x != nil {
		return x.State
	}
	return State_STATE_UNSPECIFIED
}

func (x *MachineSummary) GetTitle() string {
	if x != nil {
		return x.Title
	}
	return ""
}

func (x *MachineSummary) GetStatus() *MachineStatus {
	if x != nil {
		return x.Status
	}
	return nil
}

func (x *MachineSummary) GetAgentCount() int32 {
	if x != nil {
		return x.AgentCount
	}
	return 0
}

func (x *MachineSummary) GetCreatedBy() string {
	if x != nil {
		return x.CreatedBy
	}
	return ""
}

func (x *MachineSummary) GetCreatedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.CreatedAt
	}
	return nil
}

func (x *MachineSummary) GetCanEdit() bool {
	if x != nil {
		return x.CanEdit
	}
	return false
}

func (x *MachineSummary) GetCanManage() bool {
	if x != nil {
		return x.CanManage
	}
	return false
}

func (x *MachineSummary) GetCanDelete() bool {
	if x != nil {
		return x.CanDelete
	}
	return false
}

func (x *MachineSummary) GetLatestVersion() string {
	if x != nil {
		return x.LatestVersion
	}
	return ""
}

func (x *MachineSummary) GetUpgradeAvailable() bool {
	if x != nil {
		return x.UpgradeAvailable
	}
	return false
}

func (x *MachineSummary) GetProvisioning() *ProvisioningStatus {
	if x != nil {
		return x.Provisioning
	}
	return nil
}

// UpgradeRequest tells the machine to upgrade itself to the manager's
// embedded binary for its platform. The machine's supervisor process
// downloads the binary from the manager's /machine/bin download endpoints,
// verifies the sha256, installs it, and restarts the machine process.
type UpgradeRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// version is the binary version the machine should upgrade to (matches the
	// manager's manifest version).
	Version string `protobuf:"bytes,1,opt,name=version,proto3" json:"version,omitempty"`
	// target is the platform triple the machine should download (e.g.
	// "linux-x64"), matching the manager's manifest targets.
	Target string `protobuf:"bytes,2,opt,name=target,proto3" json:"target,omitempty"`
	// sha256 is the expected hex sha256 of the gzipped binary, from the
	// manager's manifest; the machine verifies the download against it.
	Sha256        string `protobuf:"bytes,3,opt,name=sha256,proto3" json:"sha256,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpgradeRequest) Reset() {
	*x = UpgradeRequest{}
	mi := &file_v1_machine_proto_msgTypes[34]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpgradeRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpgradeRequest) ProtoMessage() {}

func (x *UpgradeRequest) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[34]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpgradeRequest.ProtoReflect.Descriptor instead.
func (*UpgradeRequest) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{34}
}

func (x *UpgradeRequest) GetVersion() string {
	if x != nil {
		return x.Version
	}
	return ""
}

func (x *UpgradeRequest) GetTarget() string {
	if x != nil {
		return x.Target
	}
	return ""
}

func (x *UpgradeRequest) GetSha256() string {
	if x != nil {
		return x.Sha256
	}
	return ""
}

// UpgradeProgress is the machine's report on a self-upgrade. stage is one of:
// "requested", "downloading", "installing", "restarting", "done", "failed";
// error carries the failure reason when stage is "failed".
type UpgradeProgress struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Version       string                 `protobuf:"bytes,1,opt,name=version,proto3" json:"version,omitempty"`
	Stage         string                 `protobuf:"bytes,2,opt,name=stage,proto3" json:"stage,omitempty"`
	Error         string                 `protobuf:"bytes,3,opt,name=error,proto3" json:"error,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpgradeProgress) Reset() {
	*x = UpgradeProgress{}
	mi := &file_v1_machine_proto_msgTypes[35]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpgradeProgress) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpgradeProgress) ProtoMessage() {}

func (x *UpgradeProgress) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[35]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpgradeProgress.ProtoReflect.Descriptor instead.
func (*UpgradeProgress) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{35}
}

func (x *UpgradeProgress) GetVersion() string {
	if x != nil {
		return x.Version
	}
	return ""
}

func (x *UpgradeProgress) GetStage() string {
	if x != nil {
		return x.Stage
	}
	return ""
}

func (x *UpgradeProgress) GetError() string {
	if x != nil {
		return x.Error
	}
	return ""
}

// ProvisioningStatus is the provisioning state of a machine created through a
// provisioner, carried on Machine.provisioning.
type ProvisioningStatus struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	Phase ProvisioningPhase      `protobuf:"varint,1,opt,name=phase,proto3,enum=laelia.v1.ProvisioningPhase" json:"phase,omitempty"`
	// Last failure reason (phase FAILED or DEPROVISIONING issues).
	Error string `protobuf:"bytes,2,opt,name=error,proto3" json:"error,omitempty"`
	// Backend-specific workload locator, e.g. "laelia-machines/m-abc123-0".
	WorkloadName   string                 `protobuf:"bytes,3,opt,name=workload_name,json=workloadName,proto3" json:"workload_name,omitempty"`
	WorkloadLabels map[string]string      `protobuf:"bytes,4,rep,name=workload_labels,json=workloadLabels,proto3" json:"workload_labels,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	PendingAt      *timestamppb.Timestamp `protobuf:"bytes,5,opt,name=pending_at,json=pendingAt,proto3" json:"pending_at,omitempty"`
	ProvisionedAt  *timestamppb.Timestamp `protobuf:"bytes,6,opt,name=provisioned_at,json=provisionedAt,proto3" json:"provisioned_at,omitempty"`
	FailedAt       *timestamppb.Timestamp `protobuf:"bytes,7,opt,name=failed_at,json=failedAt,proto3" json:"failed_at,omitempty"`
	// The machine-specific runtime image the workload runs (custom image
	// provided at ProvisionMachine time); empty = the workspace default.
	RuntimeImage string `protobuf:"bytes,8,opt,name=runtime_image,json=runtimeImage,proto3" json:"runtime_image,omitempty"`
	// The user-provided parameter overrides persisted at ProvisionMachine time
	// (catalog-keyed, e.g. {"cpu": "2", "memory": "4Gi"}); empty = the
	// provisioner's configured defaults.
	MachineParams map[string]string `protobuf:"bytes,9,rep,name=machine_params,json=machineParams,proto3" json:"machine_params,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ProvisioningStatus) Reset() {
	*x = ProvisioningStatus{}
	mi := &file_v1_machine_proto_msgTypes[36]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ProvisioningStatus) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ProvisioningStatus) ProtoMessage() {}

func (x *ProvisioningStatus) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[36]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ProvisioningStatus.ProtoReflect.Descriptor instead.
func (*ProvisioningStatus) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{36}
}

func (x *ProvisioningStatus) GetPhase() ProvisioningPhase {
	if x != nil {
		return x.Phase
	}
	return ProvisioningPhase_PROVISIONING_PHASE_UNSPECIFIED
}

func (x *ProvisioningStatus) GetError() string {
	if x != nil {
		return x.Error
	}
	return ""
}

func (x *ProvisioningStatus) GetWorkloadName() string {
	if x != nil {
		return x.WorkloadName
	}
	return ""
}

func (x *ProvisioningStatus) GetWorkloadLabels() map[string]string {
	if x != nil {
		return x.WorkloadLabels
	}
	return nil
}

func (x *ProvisioningStatus) GetPendingAt() *timestamppb.Timestamp {
	if x != nil {
		return x.PendingAt
	}
	return nil
}

func (x *ProvisioningStatus) GetProvisionedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.ProvisionedAt
	}
	return nil
}

func (x *ProvisioningStatus) GetFailedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.FailedAt
	}
	return nil
}

func (x *ProvisioningStatus) GetRuntimeImage() string {
	if x != nil {
		return x.RuntimeImage
	}
	return ""
}

func (x *ProvisioningStatus) GetMachineParams() map[string]string {
	if x != nil {
		return x.MachineParams
	}
	return nil
}

type MachineInfo struct {
	state      protoimpl.MessageState `protogen:"open.v1"`
	Hostname   string                 `protobuf:"bytes,2,opt,name=hostname,proto3" json:"hostname,omitempty"`
	Os         string                 `protobuf:"bytes,3,opt,name=os,proto3" json:"os,omitempty"`
	Arch       string                 `protobuf:"bytes,4,opt,name=arch,proto3" json:"arch,omitempty"`
	Ip         string                 `protobuf:"bytes,5,opt,name=ip,proto3" json:"ip,omitempty"`
	Version    string                 `protobuf:"bytes,6,opt,name=version,proto3" json:"version,omitempty"`
	Labels     map[string]string      `protobuf:"bytes,7,rep,name=labels,proto3" json:"labels,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	Capability *AgentCapability       `protobuf:"bytes,8,opt,name=capability,proto3" json:"capability,omitempty"`
	// LLM agent providers auto-discovered by the machine app on its host.
	// Machine-scoped: every agent hosted on this machine selects from this list.
	AvailableProviders []*AgentProviderInfo `protobuf:"bytes,9,rep,name=available_providers,json=availableProviders,proto3" json:"available_providers,omitempty"`
	// prompt_bundle_version is the content hash of the machine binary's embedded
	// static prompt bundle (communication.md / agent_memory.md / reanchor.md /
	// AgentFirstPromptBody). The manager compares it against the expected version
	// to detect when a machine's bundled system prompt is out of date.
	PromptBundleVersion string `protobuf:"bytes,10,opt,name=prompt_bundle_version,json=promptBundleVersion,proto3" json:"prompt_bundle_version,omitempty"`
	unknownFields       protoimpl.UnknownFields
	sizeCache           protoimpl.SizeCache
}

func (x *MachineInfo) Reset() {
	*x = MachineInfo{}
	mi := &file_v1_machine_proto_msgTypes[37]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MachineInfo) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MachineInfo) ProtoMessage() {}

func (x *MachineInfo) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[37]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MachineInfo.ProtoReflect.Descriptor instead.
func (*MachineInfo) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{37}
}

func (x *MachineInfo) GetHostname() string {
	if x != nil {
		return x.Hostname
	}
	return ""
}

func (x *MachineInfo) GetOs() string {
	if x != nil {
		return x.Os
	}
	return ""
}

func (x *MachineInfo) GetArch() string {
	if x != nil {
		return x.Arch
	}
	return ""
}

func (x *MachineInfo) GetIp() string {
	if x != nil {
		return x.Ip
	}
	return ""
}

func (x *MachineInfo) GetVersion() string {
	if x != nil {
		return x.Version
	}
	return ""
}

func (x *MachineInfo) GetLabels() map[string]string {
	if x != nil {
		return x.Labels
	}
	return nil
}

func (x *MachineInfo) GetCapability() *AgentCapability {
	if x != nil {
		return x.Capability
	}
	return nil
}

func (x *MachineInfo) GetAvailableProviders() []*AgentProviderInfo {
	if x != nil {
		return x.AvailableProviders
	}
	return nil
}

func (x *MachineInfo) GetPromptBundleVersion() string {
	if x != nil {
		return x.PromptBundleVersion
	}
	return ""
}

type MachineStatus struct {
	state             protoimpl.MessageState        `protogen:"open.v1"`
	State             MachineStatus_ConnectionState `protobuf:"varint,1,opt,name=state,proto3,enum=laelia.v1.MachineStatus_ConnectionState" json:"state,omitempty"`
	LastHeartbeatTime *timestamppb.Timestamp        `protobuf:"bytes,2,opt,name=last_heartbeat_time,json=lastHeartbeatTime,proto3" json:"last_heartbeat_time,omitempty"`
	ConnectedTime     *timestamppb.Timestamp        `protobuf:"bytes,3,opt,name=connected_time,json=connectedTime,proto3" json:"connected_time,omitempty"`
	ErrorMessage      string                        `protobuf:"bytes,4,opt,name=error_message,json=errorMessage,proto3" json:"error_message,omitempty"`
	ActiveSessionId   string                        `protobuf:"bytes,5,opt,name=active_session_id,json=activeSessionId,proto3" json:"active_session_id,omitempty"`
	unknownFields     protoimpl.UnknownFields
	sizeCache         protoimpl.SizeCache
}

func (x *MachineStatus) Reset() {
	*x = MachineStatus{}
	mi := &file_v1_machine_proto_msgTypes[38]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MachineStatus) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MachineStatus) ProtoMessage() {}

func (x *MachineStatus) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[38]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MachineStatus.ProtoReflect.Descriptor instead.
func (*MachineStatus) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{38}
}

func (x *MachineStatus) GetState() MachineStatus_ConnectionState {
	if x != nil {
		return x.State
	}
	return MachineStatus_CONNECTION_STATE_UNSPECIFIED
}

func (x *MachineStatus) GetLastHeartbeatTime() *timestamppb.Timestamp {
	if x != nil {
		return x.LastHeartbeatTime
	}
	return nil
}

func (x *MachineStatus) GetConnectedTime() *timestamppb.Timestamp {
	if x != nil {
		return x.ConnectedTime
	}
	return nil
}

func (x *MachineStatus) GetErrorMessage() string {
	if x != nil {
		return x.ErrorMessage
	}
	return ""
}

func (x *MachineStatus) GetActiveSessionId() string {
	if x != nil {
		return x.ActiveSessionId
	}
	return ""
}

// AgentAssignment is the per-agent configuration the machine app needs to run an
// agent's drain loop. Pushed over the MachineChannel on CreateAgent, and sent
// in full in ConnectMachineResponse.assigned_agents on (re)connect.
type AgentAssignment struct {
	state            protoimpl.MessageState `protogen:"open.v1"`
	AgentName        string                 `protobuf:"bytes,1,opt,name=agent_name,json=agentName,proto3" json:"agent_name,omitempty"`                        // agents/{agent}
	AgentDisplayName string                 `protobuf:"bytes,2,opt,name=agent_display_name,json=agentDisplayName,proto3" json:"agent_display_name,omitempty"` // used in the init prompt
	AcpConfig        *AgentACPConfig        `protobuf:"bytes,3,opt,name=acp_config,json=acpConfig,proto3" json:"acp_config,omitempty"`                        // server-owned per-agent ACP config
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *AgentAssignment) Reset() {
	*x = AgentAssignment{}
	mi := &file_v1_machine_proto_msgTypes[39]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AgentAssignment) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AgentAssignment) ProtoMessage() {}

func (x *AgentAssignment) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[39]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AgentAssignment.ProtoReflect.Descriptor instead.
func (*AgentAssignment) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{39}
}

func (x *AgentAssignment) GetAgentName() string {
	if x != nil {
		return x.AgentName
	}
	return ""
}

func (x *AgentAssignment) GetAgentDisplayName() string {
	if x != nil {
		return x.AgentDisplayName
	}
	return ""
}

func (x *AgentAssignment) GetAcpConfig() *AgentACPConfig {
	if x != nil {
		return x.AcpConfig
	}
	return nil
}

type MachineStreamMessage struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to Message:
	//
	//	*MachineStreamMessage_MachineReady
	//	*MachineStreamMessage_Ping
	//	*MachineStreamMessage_ProvidersDiscovered
	//	*MachineStreamMessage_DisconnectNotice
	//	*MachineStreamMessage_MachineWorkspaceScanResponse
	//	*MachineStreamMessage_UpgradeProgress
	//	*MachineStreamMessage_WorkspaceListResponse
	//	*MachineStreamMessage_WorkspaceReadResponse
	//	*MachineStreamMessage_PromptReleaseNoticeAck
	//	*MachineStreamMessage_ModelsDiscovered
	//	*MachineStreamMessage_MachineMetricsResponse
	Message       isMachineStreamMessage_Message `protobuf_oneof:"message"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MachineStreamMessage) Reset() {
	*x = MachineStreamMessage{}
	mi := &file_v1_machine_proto_msgTypes[40]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MachineStreamMessage) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MachineStreamMessage) ProtoMessage() {}

func (x *MachineStreamMessage) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[40]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MachineStreamMessage.ProtoReflect.Descriptor instead.
func (*MachineStreamMessage) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{40}
}

func (x *MachineStreamMessage) GetMessage() isMachineStreamMessage_Message {
	if x != nil {
		return x.Message
	}
	return nil
}

func (x *MachineStreamMessage) GetMachineReady() *MachineReady {
	if x != nil {
		if x, ok := x.Message.(*MachineStreamMessage_MachineReady); ok {
			return x.MachineReady
		}
	}
	return nil
}

func (x *MachineStreamMessage) GetPing() *Ping {
	if x != nil {
		if x, ok := x.Message.(*MachineStreamMessage_Ping); ok {
			return x.Ping
		}
	}
	return nil
}

func (x *MachineStreamMessage) GetProvidersDiscovered() *ProvidersDiscovered {
	if x != nil {
		if x, ok := x.Message.(*MachineStreamMessage_ProvidersDiscovered); ok {
			return x.ProvidersDiscovered
		}
	}
	return nil
}

func (x *MachineStreamMessage) GetDisconnectNotice() *MachineDisconnectNotice {
	if x != nil {
		if x, ok := x.Message.(*MachineStreamMessage_DisconnectNotice); ok {
			return x.DisconnectNotice
		}
	}
	return nil
}

func (x *MachineStreamMessage) GetMachineWorkspaceScanResponse() *MachineWorkspaceScanResponse {
	if x != nil {
		if x, ok := x.Message.(*MachineStreamMessage_MachineWorkspaceScanResponse); ok {
			return x.MachineWorkspaceScanResponse
		}
	}
	return nil
}

func (x *MachineStreamMessage) GetUpgradeProgress() *UpgradeProgress {
	if x != nil {
		if x, ok := x.Message.(*MachineStreamMessage_UpgradeProgress); ok {
			return x.UpgradeProgress
		}
	}
	return nil
}

func (x *MachineStreamMessage) GetWorkspaceListResponse() *WorkspaceListResponse {
	if x != nil {
		if x, ok := x.Message.(*MachineStreamMessage_WorkspaceListResponse); ok {
			return x.WorkspaceListResponse
		}
	}
	return nil
}

func (x *MachineStreamMessage) GetWorkspaceReadResponse() *WorkspaceReadResponse {
	if x != nil {
		if x, ok := x.Message.(*MachineStreamMessage_WorkspaceReadResponse); ok {
			return x.WorkspaceReadResponse
		}
	}
	return nil
}

func (x *MachineStreamMessage) GetPromptReleaseNoticeAck() *PromptReleaseNoticeAck {
	if x != nil {
		if x, ok := x.Message.(*MachineStreamMessage_PromptReleaseNoticeAck); ok {
			return x.PromptReleaseNoticeAck
		}
	}
	return nil
}

func (x *MachineStreamMessage) GetModelsDiscovered() *ModelsDiscovered {
	if x != nil {
		if x, ok := x.Message.(*MachineStreamMessage_ModelsDiscovered); ok {
			return x.ModelsDiscovered
		}
	}
	return nil
}

func (x *MachineStreamMessage) GetMachineMetricsResponse() *MachineMetricsResponse {
	if x != nil {
		if x, ok := x.Message.(*MachineStreamMessage_MachineMetricsResponse); ok {
			return x.MachineMetricsResponse
		}
	}
	return nil
}

type isMachineStreamMessage_Message interface {
	isMachineStreamMessage_Message()
}

type MachineStreamMessage_MachineReady struct {
	MachineReady *MachineReady `protobuf:"bytes,1,opt,name=machine_ready,json=machineReady,proto3,oneof"` // first message, carries the machine session id
}

type MachineStreamMessage_Ping struct {
	Ping *Ping `protobuf:"bytes,2,opt,name=ping,proto3,oneof"`
}

type MachineStreamMessage_ProvidersDiscovered struct {
	ProvidersDiscovered *ProvidersDiscovered `protobuf:"bytes,3,opt,name=providers_discovered,json=providersDiscovered,proto3,oneof"` // response to DiscoverProviders
}

type MachineStreamMessage_DisconnectNotice struct {
	DisconnectNotice *MachineDisconnectNotice `protobuf:"bytes,4,opt,name=disconnect_notice,json=disconnectNotice,proto3,oneof"` // graceful shutdown
}

type MachineStreamMessage_MachineWorkspaceScanResponse struct {
	MachineWorkspaceScanResponse *MachineWorkspaceScanResponse `protobuf:"bytes,5,opt,name=machine_workspace_scan_response,json=machineWorkspaceScanResponse,proto3,oneof"` // response to ManagerMachineStreamMessage.machine_workspace_scan_request
}

type MachineStreamMessage_UpgradeProgress struct {
	UpgradeProgress *UpgradeProgress `protobuf:"bytes,6,opt,name=upgrade_progress,json=upgradeProgress,proto3,oneof"` // self-upgrade progress report, response to ManagerMachineStreamMessage.upgrade_request
}

type MachineStreamMessage_WorkspaceListResponse struct {
	WorkspaceListResponse *WorkspaceListResponse `protobuf:"bytes,7,opt,name=workspace_list_response,json=workspaceListResponse,proto3,oneof"` // response to ManagerMachineStreamMessage.workspace_list_request
}

type MachineStreamMessage_WorkspaceReadResponse struct {
	WorkspaceReadResponse *WorkspaceReadResponse `protobuf:"bytes,8,opt,name=workspace_read_response,json=workspaceReadResponse,proto3,oneof"` // response to ManagerMachineStreamMessage.workspace_read_request
}

type MachineStreamMessage_PromptReleaseNoticeAck struct {
	// ack that a prompt release notice was injected into a turn
	PromptReleaseNoticeAck *PromptReleaseNoticeAck `protobuf:"bytes,9,opt,name=prompt_release_notice_ack,json=promptReleaseNoticeAck,proto3,oneof"`
}

type MachineStreamMessage_ModelsDiscovered struct {
	ModelsDiscovered *ModelsDiscovered `protobuf:"bytes,10,opt,name=models_discovered,json=modelsDiscovered,proto3,oneof"` // response to ManagerMachineStreamMessage.discover_models
}

type MachineStreamMessage_MachineMetricsResponse struct {
	MachineMetricsResponse *MachineMetricsResponse `protobuf:"bytes,11,opt,name=machine_metrics_response,json=machineMetricsResponse,proto3,oneof"` // response to ManagerMachineStreamMessage.machine_metrics_request
}

func (*MachineStreamMessage_MachineReady) isMachineStreamMessage_Message() {}

func (*MachineStreamMessage_Ping) isMachineStreamMessage_Message() {}

func (*MachineStreamMessage_ProvidersDiscovered) isMachineStreamMessage_Message() {}

func (*MachineStreamMessage_DisconnectNotice) isMachineStreamMessage_Message() {}

func (*MachineStreamMessage_MachineWorkspaceScanResponse) isMachineStreamMessage_Message() {}

func (*MachineStreamMessage_UpgradeProgress) isMachineStreamMessage_Message() {}

func (*MachineStreamMessage_WorkspaceListResponse) isMachineStreamMessage_Message() {}

func (*MachineStreamMessage_WorkspaceReadResponse) isMachineStreamMessage_Message() {}

func (*MachineStreamMessage_PromptReleaseNoticeAck) isMachineStreamMessage_Message() {}

func (*MachineStreamMessage_ModelsDiscovered) isMachineStreamMessage_Message() {}

func (*MachineStreamMessage_MachineMetricsResponse) isMachineStreamMessage_Message() {}

type ManagerMachineStreamMessage struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to Message:
	//
	//	*ManagerMachineStreamMessage_AgentAssignment
	//	*ManagerMachineStreamMessage_RemoveAgent
	//	*ManagerMachineStreamMessage_AgentConfigUpdate
	//	*ManagerMachineStreamMessage_DiscoverProviders
	//	*ManagerMachineStreamMessage_Pong
	//	*ManagerMachineStreamMessage_ReloadAgentAssignment
	//	*ManagerMachineStreamMessage_MachineWorkspaceScanRequest
	//	*ManagerMachineStreamMessage_DeleteAgentWorkspace
	//	*ManagerMachineStreamMessage_UpgradeRequest
	//	*ManagerMachineStreamMessage_DiscoverModels
	//	*ManagerMachineStreamMessage_RestartAgent
	//	*ManagerMachineStreamMessage_AgentControl
	//	*ManagerMachineStreamMessage_WorkspaceListRequest
	//	*ManagerMachineStreamMessage_WorkspaceReadRequest
	//	*ManagerMachineStreamMessage_MachineMetricsRequest
	Message       isManagerMachineStreamMessage_Message `protobuf_oneof:"message"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ManagerMachineStreamMessage) Reset() {
	*x = ManagerMachineStreamMessage{}
	mi := &file_v1_machine_proto_msgTypes[41]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ManagerMachineStreamMessage) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ManagerMachineStreamMessage) ProtoMessage() {}

func (x *ManagerMachineStreamMessage) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[41]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ManagerMachineStreamMessage.ProtoReflect.Descriptor instead.
func (*ManagerMachineStreamMessage) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{41}
}

func (x *ManagerMachineStreamMessage) GetMessage() isManagerMachineStreamMessage_Message {
	if x != nil {
		return x.Message
	}
	return nil
}

func (x *ManagerMachineStreamMessage) GetAgentAssignment() *AgentAssignment {
	if x != nil {
		if x, ok := x.Message.(*ManagerMachineStreamMessage_AgentAssignment); ok {
			return x.AgentAssignment
		}
	}
	return nil
}

func (x *ManagerMachineStreamMessage) GetRemoveAgent() *RemoveAgent {
	if x != nil {
		if x, ok := x.Message.(*ManagerMachineStreamMessage_RemoveAgent); ok {
			return x.RemoveAgent
		}
	}
	return nil
}

func (x *ManagerMachineStreamMessage) GetAgentConfigUpdate() *AgentConfigUpdate {
	if x != nil {
		if x, ok := x.Message.(*ManagerMachineStreamMessage_AgentConfigUpdate); ok {
			return x.AgentConfigUpdate
		}
	}
	return nil
}

func (x *ManagerMachineStreamMessage) GetDiscoverProviders() *DiscoverProviders {
	if x != nil {
		if x, ok := x.Message.(*ManagerMachineStreamMessage_DiscoverProviders); ok {
			return x.DiscoverProviders
		}
	}
	return nil
}

func (x *ManagerMachineStreamMessage) GetPong() *Pong {
	if x != nil {
		if x, ok := x.Message.(*ManagerMachineStreamMessage_Pong); ok {
			return x.Pong
		}
	}
	return nil
}

func (x *ManagerMachineStreamMessage) GetReloadAgentAssignment() *ReloadAgentAssignment {
	if x != nil {
		if x, ok := x.Message.(*ManagerMachineStreamMessage_ReloadAgentAssignment); ok {
			return x.ReloadAgentAssignment
		}
	}
	return nil
}

func (x *ManagerMachineStreamMessage) GetMachineWorkspaceScanRequest() *MachineWorkspaceScanRequest {
	if x != nil {
		if x, ok := x.Message.(*ManagerMachineStreamMessage_MachineWorkspaceScanRequest); ok {
			return x.MachineWorkspaceScanRequest
		}
	}
	return nil
}

func (x *ManagerMachineStreamMessage) GetDeleteAgentWorkspace() *DeleteAgentWorkspace {
	if x != nil {
		if x, ok := x.Message.(*ManagerMachineStreamMessage_DeleteAgentWorkspace); ok {
			return x.DeleteAgentWorkspace
		}
	}
	return nil
}

func (x *ManagerMachineStreamMessage) GetUpgradeRequest() *UpgradeRequest {
	if x != nil {
		if x, ok := x.Message.(*ManagerMachineStreamMessage_UpgradeRequest); ok {
			return x.UpgradeRequest
		}
	}
	return nil
}

func (x *ManagerMachineStreamMessage) GetDiscoverModels() *DiscoverModels {
	if x != nil {
		if x, ok := x.Message.(*ManagerMachineStreamMessage_DiscoverModels); ok {
			return x.DiscoverModels
		}
	}
	return nil
}

func (x *ManagerMachineStreamMessage) GetRestartAgent() *RestartAgent {
	if x != nil {
		if x, ok := x.Message.(*ManagerMachineStreamMessage_RestartAgent); ok {
			return x.RestartAgent
		}
	}
	return nil
}

func (x *ManagerMachineStreamMessage) GetAgentControl() *AgentControlRequest {
	if x != nil {
		if x, ok := x.Message.(*ManagerMachineStreamMessage_AgentControl); ok {
			return x.AgentControl
		}
	}
	return nil
}

func (x *ManagerMachineStreamMessage) GetWorkspaceListRequest() *WorkspaceListRequest {
	if x != nil {
		if x, ok := x.Message.(*ManagerMachineStreamMessage_WorkspaceListRequest); ok {
			return x.WorkspaceListRequest
		}
	}
	return nil
}

func (x *ManagerMachineStreamMessage) GetWorkspaceReadRequest() *WorkspaceReadRequest {
	if x != nil {
		if x, ok := x.Message.(*ManagerMachineStreamMessage_WorkspaceReadRequest); ok {
			return x.WorkspaceReadRequest
		}
	}
	return nil
}

func (x *ManagerMachineStreamMessage) GetMachineMetricsRequest() *MachineMetricsRequest {
	if x != nil {
		if x, ok := x.Message.(*ManagerMachineStreamMessage_MachineMetricsRequest); ok {
			return x.MachineMetricsRequest
		}
	}
	return nil
}

type isManagerMachineStreamMessage_Message interface {
	isManagerMachineStreamMessage_Message()
}

type ManagerMachineStreamMessage_AgentAssignment struct {
	AgentAssignment *AgentAssignment `protobuf:"bytes,1,opt,name=agent_assignment,json=agentAssignment,proto3,oneof"` // host a new agent
}

type ManagerMachineStreamMessage_RemoveAgent struct {
	RemoveAgent *RemoveAgent `protobuf:"bytes,2,opt,name=remove_agent,json=removeAgent,proto3,oneof"` // tear down an agent's runner
}

type ManagerMachineStreamMessage_AgentConfigUpdate struct {
	AgentConfigUpdate *AgentConfigUpdate `protobuf:"bytes,3,opt,name=agent_config_update,json=agentConfigUpdate,proto3,oneof"` // hot-reload an agent's ACP config
}

type ManagerMachineStreamMessage_DiscoverProviders struct {
	DiscoverProviders *DiscoverProviders `protobuf:"bytes,4,opt,name=discover_providers,json=discoverProviders,proto3,oneof"` // ask the machine to re-probe
}

type ManagerMachineStreamMessage_Pong struct {
	Pong *Pong `protobuf:"bytes,5,opt,name=pong,proto3,oneof"`
}

type ManagerMachineStreamMessage_ReloadAgentAssignment struct {
	ReloadAgentAssignment *ReloadAgentAssignment `protobuf:"bytes,6,opt,name=reload_agent_assignment,json=reloadAgentAssignment,proto3,oneof"` // full re-sync of one agent
}

type ManagerMachineStreamMessage_MachineWorkspaceScanRequest struct {
	MachineWorkspaceScanRequest *MachineWorkspaceScanRequest `protobuf:"bytes,7,opt,name=machine_workspace_scan_request,json=machineWorkspaceScanRequest,proto3,oneof"` // scan per-agent workspace directories on this machine
}

type ManagerMachineStreamMessage_DeleteAgentWorkspace struct {
	DeleteAgentWorkspace *DeleteAgentWorkspace `protobuf:"bytes,8,opt,name=delete_agent_workspace,json=deleteAgentWorkspace,proto3,oneof"` // stop the runner and delete an agent's workspace directory
}

type ManagerMachineStreamMessage_UpgradeRequest struct {
	UpgradeRequest *UpgradeRequest `protobuf:"bytes,9,opt,name=upgrade_request,json=upgradeRequest,proto3,oneof"` // self-upgrade to the manager's embedded binary
}

type ManagerMachineStreamMessage_DiscoverModels struct {
	DiscoverModels *DiscoverModels `protobuf:"bytes,11,opt,name=discover_models,json=discoverModels,proto3,oneof"` // probe one provider's models with an env overlay
}

type ManagerMachineStreamMessage_RestartAgent struct {
	RestartAgent *RestartAgent `protobuf:"bytes,12,opt,name=restart_agent,json=restartAgent,proto3,oneof"` // force cold restart of one agent
}

type ManagerMachineStreamMessage_AgentControl struct {
	AgentControl *AgentControlRequest `protobuf:"bytes,13,opt,name=agent_control,json=agentControl,proto3,oneof"` // per-agent control interaction (cancel/steer/wake/prompt notice)
}

type ManagerMachineStreamMessage_WorkspaceListRequest struct {
	WorkspaceListRequest *WorkspaceListRequest `protobuf:"bytes,14,opt,name=workspace_list_request,json=workspaceListRequest,proto3,oneof"` // list one level of a hosted agent's workspace
}

type ManagerMachineStreamMessage_WorkspaceReadRequest struct {
	WorkspaceReadRequest *WorkspaceReadRequest `protobuf:"bytes,15,opt,name=workspace_read_request,json=workspaceReadRequest,proto3,oneof"` // read one hosted agent's workspace file
}

type ManagerMachineStreamMessage_MachineMetricsRequest struct {
	MachineMetricsRequest *MachineMetricsRequest `protobuf:"bytes,16,opt,name=machine_metrics_request,json=machineMetricsRequest,proto3,oneof"` // ask the machine to render its local metrics
}

func (*ManagerMachineStreamMessage_AgentAssignment) isManagerMachineStreamMessage_Message() {}

func (*ManagerMachineStreamMessage_RemoveAgent) isManagerMachineStreamMessage_Message() {}

func (*ManagerMachineStreamMessage_AgentConfigUpdate) isManagerMachineStreamMessage_Message() {}

func (*ManagerMachineStreamMessage_DiscoverProviders) isManagerMachineStreamMessage_Message() {}

func (*ManagerMachineStreamMessage_Pong) isManagerMachineStreamMessage_Message() {}

func (*ManagerMachineStreamMessage_ReloadAgentAssignment) isManagerMachineStreamMessage_Message() {}

func (*ManagerMachineStreamMessage_MachineWorkspaceScanRequest) isManagerMachineStreamMessage_Message() {
}

func (*ManagerMachineStreamMessage_DeleteAgentWorkspace) isManagerMachineStreamMessage_Message() {}

func (*ManagerMachineStreamMessage_UpgradeRequest) isManagerMachineStreamMessage_Message() {}

func (*ManagerMachineStreamMessage_DiscoverModels) isManagerMachineStreamMessage_Message() {}

func (*ManagerMachineStreamMessage_RestartAgent) isManagerMachineStreamMessage_Message() {}

func (*ManagerMachineStreamMessage_AgentControl) isManagerMachineStreamMessage_Message() {}

func (*ManagerMachineStreamMessage_WorkspaceListRequest) isManagerMachineStreamMessage_Message() {}

func (*ManagerMachineStreamMessage_WorkspaceReadRequest) isManagerMachineStreamMessage_Message() {}

func (*ManagerMachineStreamMessage_MachineMetricsRequest) isManagerMachineStreamMessage_Message() {}

// AgentControlRequest carries one manager→agent control interaction on the
// machine control stream. agent_name routes it to that agent's runner on the
// machine; the oneof is the per-agent control payload.
type AgentControlRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// agent_name is the agent resource name (agents/{agent}) the control
	// interaction targets; the machine validates it hosts this agent.
	AgentName string `protobuf:"bytes,1,opt,name=agent_name,json=agentName,proto3" json:"agent_name,omitempty"`
	// Types that are valid to be assigned to Control:
	//
	//	*AgentControlRequest_Cancel
	//	*AgentControlRequest_Steer
	//	*AgentControlRequest_Wake
	//	*AgentControlRequest_PromptNotice
	Control       isAgentControlRequest_Control `protobuf_oneof:"control"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AgentControlRequest) Reset() {
	*x = AgentControlRequest{}
	mi := &file_v1_machine_proto_msgTypes[42]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AgentControlRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AgentControlRequest) ProtoMessage() {}

func (x *AgentControlRequest) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[42]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AgentControlRequest.ProtoReflect.Descriptor instead.
func (*AgentControlRequest) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{42}
}

func (x *AgentControlRequest) GetAgentName() string {
	if x != nil {
		return x.AgentName
	}
	return ""
}

func (x *AgentControlRequest) GetControl() isAgentControlRequest_Control {
	if x != nil {
		return x.Control
	}
	return nil
}

func (x *AgentControlRequest) GetCancel() *CancelMessage {
	if x != nil {
		if x, ok := x.Control.(*AgentControlRequest_Cancel); ok {
			return x.Cancel
		}
	}
	return nil
}

func (x *AgentControlRequest) GetSteer() *SteerMessage {
	if x != nil {
		if x, ok := x.Control.(*AgentControlRequest_Steer); ok {
			return x.Steer
		}
	}
	return nil
}

func (x *AgentControlRequest) GetWake() *NewMessagesAvailable {
	if x != nil {
		if x, ok := x.Control.(*AgentControlRequest_Wake); ok {
			return x.Wake
		}
	}
	return nil
}

func (x *AgentControlRequest) GetPromptNotice() *PromptReleaseNotice {
	if x != nil {
		if x, ok := x.Control.(*AgentControlRequest_PromptNotice); ok {
			return x.PromptNotice
		}
	}
	return nil
}

type isAgentControlRequest_Control interface {
	isAgentControlRequest_Control()
}

type AgentControlRequest_Cancel struct {
	// cancel stops the agent's in-flight turn.
	Cancel *CancelMessage `protobuf:"bytes,2,opt,name=cancel,proto3,oneof"`
}

type AgentControlRequest_Steer struct {
	// steer injects a follow-up message into the in-flight turn.
	Steer *SteerMessage `protobuf:"bytes,3,opt,name=steer,proto3,oneof"`
}

type AgentControlRequest_Wake struct {
	// wake kicks the agent's drain loop when new work may be available.
	Wake *NewMessagesAvailable `protobuf:"bytes,4,opt,name=wake,proto3,oneof"`
}

type AgentControlRequest_PromptNotice struct {
	// prompt_notice pushes a system-prompt release notice to the agent.
	PromptNotice *PromptReleaseNotice `protobuf:"bytes,5,opt,name=prompt_notice,json=promptNotice,proto3,oneof"`
}

func (*AgentControlRequest_Cancel) isAgentControlRequest_Control() {}

func (*AgentControlRequest_Steer) isAgentControlRequest_Control() {}

func (*AgentControlRequest_Wake) isAgentControlRequest_Control() {}

func (*AgentControlRequest_PromptNotice) isAgentControlRequest_Control() {}

type MachineReady struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	SessionId     string                 `protobuf:"bytes,1,opt,name=session_id,json=sessionId,proto3" json:"session_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MachineReady) Reset() {
	*x = MachineReady{}
	mi := &file_v1_machine_proto_msgTypes[43]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MachineReady) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MachineReady) ProtoMessage() {}

func (x *MachineReady) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[43]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MachineReady.ProtoReflect.Descriptor instead.
func (*MachineReady) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{43}
}

func (x *MachineReady) GetSessionId() string {
	if x != nil {
		return x.SessionId
	}
	return ""
}

type RemoveAgent struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	AgentName     string                 `protobuf:"bytes,1,opt,name=agent_name,json=agentName,proto3" json:"agent_name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RemoveAgent) Reset() {
	*x = RemoveAgent{}
	mi := &file_v1_machine_proto_msgTypes[44]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RemoveAgent) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RemoveAgent) ProtoMessage() {}

func (x *RemoveAgent) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[44]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RemoveAgent.ProtoReflect.Descriptor instead.
func (*RemoveAgent) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{44}
}

func (x *RemoveAgent) GetAgentName() string {
	if x != nil {
		return x.AgentName
	}
	return ""
}

// RestartAgent tells the machine to force a cold restart of one agent: it
// clears the agent's persisted LLM session state and restarts its long-lived
// runtime so the next turn starts from a fresh cold start.
type RestartAgent struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	AgentName     string                 `protobuf:"bytes,1,opt,name=agent_name,json=agentName,proto3" json:"agent_name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RestartAgent) Reset() {
	*x = RestartAgent{}
	mi := &file_v1_machine_proto_msgTypes[45]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RestartAgent) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RestartAgent) ProtoMessage() {}

func (x *RestartAgent) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[45]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RestartAgent.ProtoReflect.Descriptor instead.
func (*RestartAgent) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{45}
}

func (x *RestartAgent) GetAgentName() string {
	if x != nil {
		return x.AgentName
	}
	return ""
}

// DeleteAgentWorkspace tells the machine to tear down an agent's runner and
// permanently remove its workspace directory under the machine data root.
type DeleteAgentWorkspace struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	AgentName     string                 `protobuf:"bytes,1,opt,name=agent_name,json=agentName,proto3" json:"agent_name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteAgentWorkspace) Reset() {
	*x = DeleteAgentWorkspace{}
	mi := &file_v1_machine_proto_msgTypes[46]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteAgentWorkspace) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteAgentWorkspace) ProtoMessage() {}

func (x *DeleteAgentWorkspace) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[46]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteAgentWorkspace.ProtoReflect.Descriptor instead.
func (*DeleteAgentWorkspace) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{46}
}

func (x *DeleteAgentWorkspace) GetAgentName() string {
	if x != nil {
		return x.AgentName
	}
	return ""
}

type AgentConfigUpdate struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	AgentName     string                 `protobuf:"bytes,1,opt,name=agent_name,json=agentName,proto3" json:"agent_name,omitempty"`
	AcpConfig     *AgentACPConfig        `protobuf:"bytes,2,opt,name=acp_config,json=acpConfig,proto3" json:"acp_config,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AgentConfigUpdate) Reset() {
	*x = AgentConfigUpdate{}
	mi := &file_v1_machine_proto_msgTypes[47]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AgentConfigUpdate) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AgentConfigUpdate) ProtoMessage() {}

func (x *AgentConfigUpdate) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[47]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AgentConfigUpdate.ProtoReflect.Descriptor instead.
func (*AgentConfigUpdate) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{47}
}

func (x *AgentConfigUpdate) GetAgentName() string {
	if x != nil {
		return x.AgentName
	}
	return ""
}

func (x *AgentConfigUpdate) GetAcpConfig() *AgentACPConfig {
	if x != nil {
		return x.AcpConfig
	}
	return nil
}

type MachineDisconnectNotice struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Reason        string                 `protobuf:"bytes,1,opt,name=reason,proto3" json:"reason,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MachineDisconnectNotice) Reset() {
	*x = MachineDisconnectNotice{}
	mi := &file_v1_machine_proto_msgTypes[48]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MachineDisconnectNotice) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MachineDisconnectNotice) ProtoMessage() {}

func (x *MachineDisconnectNotice) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[48]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MachineDisconnectNotice.ProtoReflect.Descriptor instead.
func (*MachineDisconnectNotice) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{48}
}

func (x *MachineDisconnectNotice) GetReason() string {
	if x != nil {
		return x.Reason
	}
	return ""
}

// DiscoverModels asks the machine app to probe a single provider's models with
// an env overlay (the agent's custom_env, e.g. CODEX_HOME pointing at a
// profile-specific codex home). The machine replies with
// MachineStreamMessage.models_discovered. Used by the unary RefreshAgentModels
// RPC so the model picker reflects an agent's custom env before saving.
type DiscoverModels struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	RequestId     string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"`                                              // correlation id for the pending RefreshAgentModels call
	Provider      string                 `protobuf:"bytes,2,opt,name=provider,proto3" json:"provider,omitempty"`                                                                 // provider id to probe, e.g. "codex"
	Env           map[string]string      `protobuf:"bytes,3,rep,name=env,proto3" json:"env,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"` // KEY=VALUE overlay applied on top of the host env
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DiscoverModels) Reset() {
	*x = DiscoverModels{}
	mi := &file_v1_machine_proto_msgTypes[49]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DiscoverModels) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DiscoverModels) ProtoMessage() {}

func (x *DiscoverModels) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[49]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DiscoverModels.ProtoReflect.Descriptor instead.
func (*DiscoverModels) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{49}
}

func (x *DiscoverModels) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *DiscoverModels) GetProvider() string {
	if x != nil {
		return x.Provider
	}
	return ""
}

func (x *DiscoverModels) GetEnv() map[string]string {
	if x != nil {
		return x.Env
	}
	return nil
}

// ModelsDiscovered carries one provider's freshly probed models back to the
// manager, which hands them to the pending RefreshAgentModels caller.
type ModelsDiscovered struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	RequestId     string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"`
	Provider      string                 `protobuf:"bytes,2,opt,name=provider,proto3" json:"provider,omitempty"`
	Models        []*AgentModelOption    `protobuf:"bytes,3,rep,name=models,proto3" json:"models,omitempty"`
	Error         string                 `protobuf:"bytes,4,opt,name=error,proto3" json:"error,omitempty"` // probe failure message; empty on success
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ModelsDiscovered) Reset() {
	*x = ModelsDiscovered{}
	mi := &file_v1_machine_proto_msgTypes[50]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ModelsDiscovered) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ModelsDiscovered) ProtoMessage() {}

func (x *ModelsDiscovered) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[50]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ModelsDiscovered.ProtoReflect.Descriptor instead.
func (*ModelsDiscovered) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{50}
}

func (x *ModelsDiscovered) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *ModelsDiscovered) GetProvider() string {
	if x != nil {
		return x.Provider
	}
	return ""
}

func (x *ModelsDiscovered) GetModels() []*AgentModelOption {
	if x != nil {
		return x.Models
	}
	return nil
}

func (x *ModelsDiscovered) GetError() string {
	if x != nil {
		return x.Error
	}
	return ""
}

// ReloadAgentAssignment is a full re-sync of a single agent's assignment (used
// after a config or display-name change, or to re-establish a runner).
type ReloadAgentAssignment struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	AgentName     string                 `protobuf:"bytes,1,opt,name=agent_name,json=agentName,proto3" json:"agent_name,omitempty"`
	Assignment    *AgentAssignment       `protobuf:"bytes,2,opt,name=assignment,proto3" json:"assignment,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ReloadAgentAssignment) Reset() {
	*x = ReloadAgentAssignment{}
	mi := &file_v1_machine_proto_msgTypes[51]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ReloadAgentAssignment) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ReloadAgentAssignment) ProtoMessage() {}

func (x *ReloadAgentAssignment) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[51]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ReloadAgentAssignment.ProtoReflect.Descriptor instead.
func (*ReloadAgentAssignment) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{51}
}

func (x *ReloadAgentAssignment) GetAgentName() string {
	if x != nil {
		return x.AgentName
	}
	return ""
}

func (x *ReloadAgentAssignment) GetAssignment() *AgentAssignment {
	if x != nil {
		return x.Assignment
	}
	return nil
}

// MachineWorkspaceScanRequest asks the machine app to summarize every
// per-agent workspace directory under ~/.laelia/<machineID>/. The app replies
// with MachineStreamMessage.machine_workspace_scan_response.
type MachineWorkspaceScanRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	RequestId     string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"` // correlation id for the pending unary ListMachineWorkspaces call
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MachineWorkspaceScanRequest) Reset() {
	*x = MachineWorkspaceScanRequest{}
	mi := &file_v1_machine_proto_msgTypes[52]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MachineWorkspaceScanRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MachineWorkspaceScanRequest) ProtoMessage() {}

func (x *MachineWorkspaceScanRequest) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[52]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MachineWorkspaceScanRequest.ProtoReflect.Descriptor instead.
func (*MachineWorkspaceScanRequest) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{52}
}

func (x *MachineWorkspaceScanRequest) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

// MachineWorkspaceSummary is one agent workspace directory's usage summary.
type MachineWorkspaceSummary struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	DirectoryName  string                 `protobuf:"bytes,1,opt,name=directory_name,json=directoryName,proto3" json:"directory_name,omitempty"`
	TotalSizeBytes int64                  `protobuf:"varint,2,opt,name=total_size_bytes,json=totalSizeBytes,proto3" json:"total_size_bytes,omitempty"`
	LastModified   *timestamppb.Timestamp `protobuf:"bytes,3,opt,name=last_modified,json=lastModified,proto3" json:"last_modified,omitempty"`
	FileCount      int64                  `protobuf:"varint,4,opt,name=file_count,json=fileCount,proto3" json:"file_count,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *MachineWorkspaceSummary) Reset() {
	*x = MachineWorkspaceSummary{}
	mi := &file_v1_machine_proto_msgTypes[53]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MachineWorkspaceSummary) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MachineWorkspaceSummary) ProtoMessage() {}

func (x *MachineWorkspaceSummary) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[53]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MachineWorkspaceSummary.ProtoReflect.Descriptor instead.
func (*MachineWorkspaceSummary) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{53}
}

func (x *MachineWorkspaceSummary) GetDirectoryName() string {
	if x != nil {
		return x.DirectoryName
	}
	return ""
}

func (x *MachineWorkspaceSummary) GetTotalSizeBytes() int64 {
	if x != nil {
		return x.TotalSizeBytes
	}
	return 0
}

func (x *MachineWorkspaceSummary) GetLastModified() *timestamppb.Timestamp {
	if x != nil {
		return x.LastModified
	}
	return nil
}

func (x *MachineWorkspaceSummary) GetFileCount() int64 {
	if x != nil {
		return x.FileCount
	}
	return 0
}

type MachineWorkspaceScanResponse struct {
	state         protoimpl.MessageState     `protogen:"open.v1"`
	RequestId     string                     `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"`
	Workspaces    []*MachineWorkspaceSummary `protobuf:"bytes,2,rep,name=workspaces,proto3" json:"workspaces,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MachineWorkspaceScanResponse) Reset() {
	*x = MachineWorkspaceScanResponse{}
	mi := &file_v1_machine_proto_msgTypes[54]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MachineWorkspaceScanResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MachineWorkspaceScanResponse) ProtoMessage() {}

func (x *MachineWorkspaceScanResponse) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[54]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MachineWorkspaceScanResponse.ProtoReflect.Descriptor instead.
func (*MachineWorkspaceScanResponse) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{54}
}

func (x *MachineWorkspaceScanResponse) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *MachineWorkspaceScanResponse) GetWorkspaces() []*MachineWorkspaceSummary {
	if x != nil {
		return x.Workspaces
	}
	return nil
}

// MachineMetricsRequest asks a connected machine to render its local metrics
// (outbox lag/bytes, upload batching, barrier waits; design §8.2) in the
// Prometheus text exposition format. The machine app replies with
// MachineStreamMessage.machine_metrics_response.
type MachineMetricsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	RequestId     string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"` // correlation id for the pending GetMachineMetrics call
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MachineMetricsRequest) Reset() {
	*x = MachineMetricsRequest{}
	mi := &file_v1_machine_proto_msgTypes[55]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MachineMetricsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MachineMetricsRequest) ProtoMessage() {}

func (x *MachineMetricsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[55]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MachineMetricsRequest.ProtoReflect.Descriptor instead.
func (*MachineMetricsRequest) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{55}
}

func (x *MachineMetricsRequest) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

// MachineMetricsResponse carries one machine's rendered Prometheus text
// exposition payload back to the pending GetMachineMetrics caller.
type MachineMetricsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	RequestId     string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"`
	Payload       string                 `protobuf:"bytes,2,opt,name=payload,proto3" json:"payload,omitempty"` // Prometheus text exposition format; empty on error
	Error         string                 `protobuf:"bytes,3,opt,name=error,proto3" json:"error,omitempty"`     // render failure message; empty on success
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MachineMetricsResponse) Reset() {
	*x = MachineMetricsResponse{}
	mi := &file_v1_machine_proto_msgTypes[56]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MachineMetricsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MachineMetricsResponse) ProtoMessage() {}

func (x *MachineMetricsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[56]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MachineMetricsResponse.ProtoReflect.Descriptor instead.
func (*MachineMetricsResponse) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{56}
}

func (x *MachineMetricsResponse) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *MachineMetricsResponse) GetPayload() string {
	if x != nil {
		return x.Payload
	}
	return ""
}

func (x *MachineMetricsResponse) GetError() string {
	if x != nil {
		return x.Error
	}
	return ""
}

// GetMachineMetricsRequest names the machine whose local metrics to scrape.
type GetMachineMetricsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Name          string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"` // machines/{machine}
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetMachineMetricsRequest) Reset() {
	*x = GetMachineMetricsRequest{}
	mi := &file_v1_machine_proto_msgTypes[57]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetMachineMetricsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetMachineMetricsRequest) ProtoMessage() {}

func (x *GetMachineMetricsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[57]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetMachineMetricsRequest.ProtoReflect.Descriptor instead.
func (*GetMachineMetricsRequest) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{57}
}

func (x *GetMachineMetricsRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

// GetMachineMetricsResponse carries the machine's rendered metrics payload.
type GetMachineMetricsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Payload       string                 `protobuf:"bytes,1,opt,name=payload,proto3" json:"payload,omitempty"` // Prometheus text exposition format
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetMachineMetricsResponse) Reset() {
	*x = GetMachineMetricsResponse{}
	mi := &file_v1_machine_proto_msgTypes[58]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetMachineMetricsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetMachineMetricsResponse) ProtoMessage() {}

func (x *GetMachineMetricsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_v1_machine_proto_msgTypes[58]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetMachineMetricsResponse.ProtoReflect.Descriptor instead.
func (*GetMachineMetricsResponse) Descriptor() ([]byte, []int) {
	return file_v1_machine_proto_rawDescGZIP(), []int{58}
}

func (x *GetMachineMetricsResponse) GetPayload() string {
	if x != nil {
		return x.Payload
	}
	return ""
}

var File_v1_machine_proto protoreflect.FileDescriptor

const file_v1_machine_proto_rawDesc = "" +
	"\n" +
	"\x10v1/machine.proto\x12\tlaelia.v1\x1a\x17google/api/client.proto\x1a\x1fgoogle/api/field_behavior.proto\x1a\x19google/api/resource.proto\x1a\x1bgoogle/protobuf/empty.proto\x1a\x1fgoogle/protobuf/timestamp.proto\x1a\x0ev1/agent.proto\x1a\x13v1/annotation.proto\x1a\x10v1/command.proto\x1a\x0fv1/common.proto\"4\n" +
	"\x13BeginSessionRequest\x12\x1d\n" +
	"\n" +
	"agent_name\x18\x01 \x01(\tR\tagentName\"\xf6\x02\n" +
	"\x16UploadCommandDataEntry\x12\x1d\n" +
	"\n" +
	"command_id\x18\x01 \x01(\tR\tcommandId\x12.\n" +
	"\x04kind\x18\x02 \x01(\x0e2\x1a.laelia.v1.UploadEntryKindR\x04kind\x12\x15\n" +
	"\x06seq_no\x18\x03 \x01(\x05R\x05seqNo\x12L\n" +
	"\x14agent_side_timestamp\x18\x04 \x01(\v2\x1a.google.protobuf.TimestampR\x12agentSideTimestamp\x128\n" +
	"\bprogress\x18\x05 \x01(\v2\x1a.laelia.v1.CommandProgressH\x00R\bprogress\x12/\n" +
	"\x05event\x18\x06 \x01(\v2\x17.laelia.v1.CommandEventH\x00R\x05event\x122\n" +
	"\x06result\x18\a \x01(\v2\x18.laelia.v1.CommandResultH\x00R\x06resultB\t\n" +
	"\apayload\"W\n" +
	"\x18UploadCommandDataRequest\x12;\n" +
	"\aentries\x18\x01 \x03(\v2!.laelia.v1.UploadCommandDataEntryR\aentries\"\xaa\x01\n" +
	"\x14UploadCommandDataAck\x12\x1d\n" +
	"\n" +
	"command_id\x18\x01 \x01(\tR\tcommandId\x12*\n" +
	"\x11last_progress_seq\x18\x02 \x01(\x05R\x0flastProgressSeq\x12$\n" +
	"\x0elast_event_seq\x18\x03 \x01(\x05R\flastEventSeq\x12!\n" +
	"\fresult_acked\x18\x04 \x01(\bR\vresultAcked\"\x9a\x01\n" +
	"\x1aUploadCommandDataRejection\x12\x1d\n" +
	"\n" +
	"command_id\x18\x01 \x01(\tR\tcommandId\x12.\n" +
	"\x04kind\x18\x02 \x01(\x0e2\x1a.laelia.v1.UploadEntryKindR\x04kind\x12\x15\n" +
	"\x06seq_no\x18\x03 \x01(\x05R\x05seqNo\x12\x16\n" +
	"\x06reason\x18\x04 \x01(\tR\x06reason\"\x93\x01\n" +
	"\x19UploadCommandDataResponse\x123\n" +
	"\x04acks\x18\x01 \x03(\v2\x1f.laelia.v1.UploadCommandDataAckR\x04acks\x12A\n" +
	"\brejected\x18\x02 \x03(\v2%.laelia.v1.UploadCommandDataRejectionR\brejected\"X\n" +
	"\x14UpdateMachineRequest\x12*\n" +
	"\x04name\x18\x01 \x01(\tB\x16\xe0A\x02\xfaA\x10\n" +
	"\x0elaelia/MachineR\x04name\x12\x14\n" +
	"\x05title\x18\x02 \x01(\tR\x05title\"\x87\x01\n" +
	"\x1fTransferMachineOwnershipRequest\x12*\n" +
	"\x04name\x18\x01 \x01(\tB\x16\xe0A\x02\xfaA\x10\n" +
	"\x0elaelia/MachineR\x04name\x12 \n" +
	"\tnew_owner\x18\x02 \x01(\tB\x03\xe0A\x02R\bnewOwner\x12\x16\n" +
	"\x06reason\x18\x03 \x01(\tR\x06reason\"P\n" +
	" TransferMachineOwnershipResponse\x12,\n" +
	"\amachine\x18\x01 \x01(\v2\x12.laelia.v1.MachineR\amachine\"_\n" +
	"\x19RevokeMachineTokenRequest\x12*\n" +
	"\x04name\x18\x01 \x01(\tB\x16\xe0A\x02\xfaA\x10\n" +
	"\x0elaelia/MachineR\x04name\x12\x16\n" +
	"\x06reason\x18\x02 \x01(\tR\x06reason\"\x1c\n" +
	"\x1aRevokeMachineTokenResponse\"c\n" +
	"\x1dForceDisconnectMachineRequest\x12*\n" +
	"\x04name\x18\x01 \x01(\tB\x16\xe0A\x02\xfaA\x10\n" +
	"\x0elaelia/MachineR\x04name\x12\x16\n" +
	"\x06reason\x18\x02 \x01(\tR\x06reason\"\x82\x01\n" +
	"\x18ListMachineAgentsRequest\x12*\n" +
	"\x04name\x18\x01 \x01(\tB\x16\xe0A\x02\xfaA\x10\n" +
	"\x0elaelia/MachineR\x04name\x12\x1b\n" +
	"\tpage_size\x18\x02 \x01(\x05R\bpageSize\x12\x1d\n" +
	"\n" +
	"page_token\x18\x03 \x01(\tR\tpageToken\"t\n" +
	"\x19ListMachineAgentsResponse\x12/\n" +
	"\x06agents\x18\x01 \x03(\v2\x17.laelia.v1.AgentSummaryR\x06agents\x12&\n" +
	"\x0fnext_page_token\x18\x02 \x01(\tR\rnextPageToken\"L\n" +
	"\x1eRefreshMachineProvidersRequest\x12*\n" +
	"\x04name\x18\x01 \x01(\tB\x16\xe0A\x02\xfaA\x10\n" +
	"\x0elaelia/MachineR\x04name\"]\n" +
	"\x1fRefreshMachineProvidersResponse\x12:\n" +
	"\tproviders\x18\x01 \x03(\v2\x1c.laelia.v1.AgentProviderInfoR\tproviders\"\x83\x01\n" +
	"\x1bRefreshMachineModelsRequest\x12*\n" +
	"\x04name\x18\x01 \x01(\tB\x16\xe0A\x02\xfaA\x10\n" +
	"\x0elaelia/MachineR\x04name\x128\n" +
	"\n" +
	"acp_config\x18\x02 \x01(\v2\x19.laelia.v1.AgentACPConfigR\tacpConfig\"\x85\x01\n" +
	"\x1cRefreshMachineModelsResponse\x12\x1a\n" +
	"\bprovider\x18\x01 \x01(\tR\bprovider\x123\n" +
	"\x06models\x18\x02 \x03(\v2\x1b.laelia.v1.AgentModelOptionR\x06models\x12\x14\n" +
	"\x05error\x18\x03 \x01(\tR\x05error\"[\n" +
	"\x15UpgradeMachineRequest\x12*\n" +
	"\x04name\x18\x01 \x01(\tB\x16\xe0A\x02\xfaA\x10\n" +
	"\x0elaelia/MachineR\x04name\x12\x16\n" +
	"\x06reason\x18\x02 \x01(\tR\x06reason\"J\n" +
	"\x1cListMachineWorkspacesRequest\x12*\n" +
	"\x04name\x18\x01 \x01(\tB\x16\xe0A\x02\xfaA\x10\n" +
	"\x0elaelia/MachineR\x04name\"c\n" +
	"\x1dListMachineWorkspacesResponse\x12B\n" +
	"\n" +
	"workspaces\x18\x01 \x03(\v2\".laelia.v1.MachineWorkspaceSummaryR\n" +
	"workspaces\"e\n" +
	"\x15ConnectMachineRequest\x12*\n" +
	"\x04info\x18\x01 \x01(\v2\x16.laelia.v1.MachineInfoR\x04info\x12 \n" +
	"\vfingerprint\x18\x02 \x01(\tR\vfingerprint\"\xbd\x01\n" +
	"\x16ConnectMachineResponse\x12\x1d\n" +
	"\n" +
	"session_id\x18\x01 \x01(\tR\tsessionId\x12?\n" +
	"\x0einitial_status\x18\x02 \x01(\v2\x18.laelia.v1.MachineStatusR\rinitialStatus\x12C\n" +
	"\x0fassigned_agents\x18\x03 \x03(\v2\x1a.laelia.v1.AgentAssignmentR\x0eassignedAgents\"_\n" +
	"\x17MachineHeartbeatRequest\x12\x1d\n" +
	"\n" +
	"session_id\x18\x01 \x01(\tR\tsessionId\x12%\n" +
	"\x0eprevious_nonce\x18\x02 \x01(\tR\rpreviousNonce\"\xf7\x01\n" +
	"\x18MachineHeartbeatResponse\x12\x1d\n" +
	"\n" +
	"next_nonce\x18\x01 \x01(\tR\tnextNonce\x12F\n" +
	"\x11next_heartbeat_at\x18\x02 \x01(\v2\x1a.google.protobuf.TimestampR\x0fnextHeartbeatAt\x12!\n" +
	"\faccess_token\x18\x03 \x01(\tR\vaccessToken\x12Q\n" +
	"\x17access_token_expires_at\x18\x04 \x01(\v2\x1a.google.protobuf.TimestampR\x14accessTokenExpiresAt\"Q\n" +
	"\x18MachineDisconnectRequest\x12\x1d\n" +
	"\n" +
	"session_id\x18\x01 \x01(\tR\tsessionId\x12\x16\n" +
	"\x06reason\x18\x02 \x01(\tR\x06reason\"c\n" +
	"\x1aRefreshMachineTokenRequest\x12#\n" +
	"\rrefresh_token\x18\x01 \x01(\tR\frefreshToken\x12 \n" +
	"\vfingerprint\x18\x02 \x01(\tR\vfingerprint\"\xb8\x01\n" +
	"\x1bRefreshMachineTokenResponse\x12!\n" +
	"\faccess_token\x18\x01 \x01(\tR\vaccessToken\x12#\n" +
	"\rrefresh_token\x18\x02 \x01(\tR\frefreshToken\x12Q\n" +
	"\x17access_token_expires_at\x18\x03 \x01(\v2\x1a.google.protobuf.TimestampR\x14accessTokenExpiresAt\"\xaf\x01\n" +
	"\x13ListMachinesRequest\x12\x1b\n" +
	"\tpage_size\x18\x01 \x01(\x05R\bpageSize\x12\x1d\n" +
	"\n" +
	"page_token\x18\x02 \x01(\tR\tpageToken\x12!\n" +
	"\fshow_deleted\x18\x03 \x01(\bR\vshowDeleted\x129\n" +
	"\vprovisioner\x18\x04 \x01(\tB\x17\xfaA\x14\n" +
	"\x12laelia/ProvisionerR\vprovisioner\"u\n" +
	"\x14ListMachinesResponse\x125\n" +
	"\bmachines\x18\x01 \x03(\v2\x19.laelia.v1.MachineSummaryR\bmachines\x12&\n" +
	"\x0fnext_page_token\x18\x02 \x01(\tR\rnextPageToken\"?\n" +
	"\x11GetMachineRequest\x12*\n" +
	"\x04name\x18\x01 \x01(\tB\x16\xe0A\x02\xfaA\x10\n" +
	"\x0elaelia/MachineR\x04name\"B\n" +
	"\x14DeleteMachineRequest\x12*\n" +
	"\x04name\x18\x01 \x01(\tB\x16\xe0A\x02\xfaA\x10\n" +
	"\x0elaelia/MachineR\x04name\"\xc2\x06\n" +
	"\aMachine\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12&\n" +
	"\x05state\x18\x02 \x01(\x0e2\x10.laelia.v1.StateR\x05state\x12\x14\n" +
	"\x05title\x18\x03 \x01(\tR\x05title\x12*\n" +
	"\x04info\x18\x05 \x01(\v2\x16.laelia.v1.MachineInfoR\x04info\x120\n" +
	"\x06status\x18\x06 \x01(\v2\x18.laelia.v1.MachineStatusR\x06status\x129\n" +
	"\n" +
	"created_at\x18\a \x01(\v2\x1a.google.protobuf.TimestampR\tcreatedAt\x126\n" +
	"\x06labels\x18\b \x03(\v2\x1e.laelia.v1.Machine.LabelsEntryR\x06labels\x12\"\n" +
	"\n" +
	"created_by\x18\v \x01(\tB\x03\xe0A\x03R\tcreatedBy\x12\x1e\n" +
	"\bcan_edit\x18\f \x01(\bB\x03\xe0A\x03R\acanEdit\x12-\n" +
	"\x10can_create_agent\x18\r \x01(\bB\x03\xe0A\x03R\x0ecanCreateAgent\x12\"\n" +
	"\n" +
	"can_manage\x18\x0e \x01(\bB\x03\xe0A\x03R\tcanManage\x12*\n" +
	"\x0elatest_version\x18\x0f \x01(\tB\x03\xe0A\x03R\rlatestVersion\x120\n" +
	"\x11upgrade_available\x18\x10 \x01(\bB\x03\xe0A\x03R\x10upgradeAvailable\x12F\n" +
	"\x0eupgrade_status\x18\x11 \x01(\v2\x1a.laelia.v1.UpgradeProgressB\x03\xe0A\x03R\rupgradeStatus\x12F\n" +
	"\fprovisioning\x18\x12 \x01(\v2\x1d.laelia.v1.ProvisioningStatusB\x03\xe0A\x03R\fprovisioning\x12%\n" +
	"\vprovisioner\x18\x13 \x01(\tB\x03\xe0A\x03R\vprovisioner\x1a9\n" +
	"\vLabelsEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01:'\xeaA$\n" +
	"\x0elaelia/Machine\x12\x12machines/{machine}J\x04\b\x04\x10\x05\"\xa2\x04\n" +
	"\x0eMachineSummary\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12&\n" +
	"\x05state\x18\x02 \x01(\x0e2\x10.laelia.v1.StateR\x05state\x12\x14\n" +
	"\x05title\x18\x03 \x01(\tR\x05title\x120\n" +
	"\x06status\x18\x04 \x01(\v2\x18.laelia.v1.MachineStatusR\x06status\x12\x1f\n" +
	"\vagent_count\x18\x05 \x01(\x05R\n" +
	"agentCount\x12\"\n" +
	"\n" +
	"created_by\x18\x06 \x01(\tB\x03\xe0A\x03R\tcreatedBy\x129\n" +
	"\n" +
	"created_at\x18\n" +
	" \x01(\v2\x1a.google.protobuf.TimestampR\tcreatedAt\x12\x1e\n" +
	"\bcan_edit\x18\a \x01(\bB\x03\xe0A\x03R\acanEdit\x12\"\n" +
	"\n" +
	"can_manage\x18\b \x01(\bB\x03\xe0A\x03R\tcanManage\x12\"\n" +
	"\n" +
	"can_delete\x18\t \x01(\bB\x03\xe0A\x03R\tcanDelete\x12*\n" +
	"\x0elatest_version\x18\v \x01(\tB\x03\xe0A\x03R\rlatestVersion\x120\n" +
	"\x11upgrade_available\x18\f \x01(\bB\x03\xe0A\x03R\x10upgradeAvailable\x12F\n" +
	"\fprovisioning\x18\r \x01(\v2\x1d.laelia.v1.ProvisioningStatusB\x03\xe0A\x03R\fprovisioning\"Z\n" +
	"\x0eUpgradeRequest\x12\x18\n" +
	"\aversion\x18\x01 \x01(\tR\aversion\x12\x16\n" +
	"\x06target\x18\x02 \x01(\tR\x06target\x12\x16\n" +
	"\x06sha256\x18\x03 \x01(\tR\x06sha256\"W\n" +
	"\x0fUpgradeProgress\x12\x18\n" +
	"\aversion\x18\x01 \x01(\tR\aversion\x12\x14\n" +
	"\x05stage\x18\x02 \x01(\tR\x05stage\x12\x14\n" +
	"\x05error\x18\x03 \x01(\tR\x05error\"\x99\x05\n" +
	"\x12ProvisioningStatus\x122\n" +
	"\x05phase\x18\x01 \x01(\x0e2\x1c.laelia.v1.ProvisioningPhaseR\x05phase\x12\x14\n" +
	"\x05error\x18\x02 \x01(\tR\x05error\x12#\n" +
	"\rworkload_name\x18\x03 \x01(\tR\fworkloadName\x12Z\n" +
	"\x0fworkload_labels\x18\x04 \x03(\v21.laelia.v1.ProvisioningStatus.WorkloadLabelsEntryR\x0eworkloadLabels\x129\n" +
	"\n" +
	"pending_at\x18\x05 \x01(\v2\x1a.google.protobuf.TimestampR\tpendingAt\x12A\n" +
	"\x0eprovisioned_at\x18\x06 \x01(\v2\x1a.google.protobuf.TimestampR\rprovisionedAt\x127\n" +
	"\tfailed_at\x18\a \x01(\v2\x1a.google.protobuf.TimestampR\bfailedAt\x12#\n" +
	"\rruntime_image\x18\b \x01(\tR\fruntimeImage\x12W\n" +
	"\x0emachine_params\x18\t \x03(\v20.laelia.v1.ProvisioningStatus.MachineParamsEntryR\rmachineParams\x1aA\n" +
	"\x13WorkloadLabelsEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\x1a@\n" +
	"\x12MachineParamsEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\"\xad\x03\n" +
	"\vMachineInfo\x12\x1a\n" +
	"\bhostname\x18\x02 \x01(\tR\bhostname\x12\x0e\n" +
	"\x02os\x18\x03 \x01(\tR\x02os\x12\x12\n" +
	"\x04arch\x18\x04 \x01(\tR\x04arch\x12\x0e\n" +
	"\x02ip\x18\x05 \x01(\tR\x02ip\x12\x18\n" +
	"\aversion\x18\x06 \x01(\tR\aversion\x12:\n" +
	"\x06labels\x18\a \x03(\v2\".laelia.v1.MachineInfo.LabelsEntryR\x06labels\x12:\n" +
	"\n" +
	"capability\x18\b \x01(\v2\x1a.laelia.v1.AgentCapabilityR\n" +
	"capability\x12M\n" +
	"\x13available_providers\x18\t \x03(\v2\x1c.laelia.v1.AgentProviderInfoR\x12availableProviders\x122\n" +
	"\x15prompt_bundle_version\x18\n" +
	" \x01(\tR\x13promptBundleVersion\x1a9\n" +
	"\vLabelsEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\"\x94\x03\n" +
	"\rMachineStatus\x12>\n" +
	"\x05state\x18\x01 \x01(\x0e2(.laelia.v1.MachineStatus.ConnectionStateR\x05state\x12J\n" +
	"\x13last_heartbeat_time\x18\x02 \x01(\v2\x1a.google.protobuf.TimestampR\x11lastHeartbeatTime\x12A\n" +
	"\x0econnected_time\x18\x03 \x01(\v2\x1a.google.protobuf.TimestampR\rconnectedTime\x12#\n" +
	"\rerror_message\x18\x04 \x01(\tR\ferrorMessage\x12*\n" +
	"\x11active_session_id\x18\x05 \x01(\tR\x0factiveSessionId\"c\n" +
	"\x0fConnectionState\x12 \n" +
	"\x1cCONNECTION_STATE_UNSPECIFIED\x10\x00\x12\n" +
	"\n" +
	"\x06ONLINE\x10\x01\x12\v\n" +
	"\aOFFLINE\x10\x02\x12\t\n" +
	"\x05ERROR\x10\x03\x12\n" +
	"\n" +
	"\x06KICKED\x10\x04\"\x98\x01\n" +
	"\x0fAgentAssignment\x12\x1d\n" +
	"\n" +
	"agent_name\x18\x01 \x01(\tR\tagentName\x12,\n" +
	"\x12agent_display_name\x18\x02 \x01(\tR\x10agentDisplayName\x128\n" +
	"\n" +
	"acp_config\x18\x03 \x01(\v2\x19.laelia.v1.AgentACPConfigR\tacpConfig\"\xae\a\n" +
	"\x14MachineStreamMessage\x12>\n" +
	"\rmachine_ready\x18\x01 \x01(\v2\x17.laelia.v1.MachineReadyH\x00R\fmachineReady\x12%\n" +
	"\x04ping\x18\x02 \x01(\v2\x0f.laelia.v1.PingH\x00R\x04ping\x12S\n" +
	"\x14providers_discovered\x18\x03 \x01(\v2\x1e.laelia.v1.ProvidersDiscoveredH\x00R\x13providersDiscovered\x12Q\n" +
	"\x11disconnect_notice\x18\x04 \x01(\v2\".laelia.v1.MachineDisconnectNoticeH\x00R\x10disconnectNotice\x12p\n" +
	"\x1fmachine_workspace_scan_response\x18\x05 \x01(\v2'.laelia.v1.MachineWorkspaceScanResponseH\x00R\x1cmachineWorkspaceScanResponse\x12G\n" +
	"\x10upgrade_progress\x18\x06 \x01(\v2\x1a.laelia.v1.UpgradeProgressH\x00R\x0fupgradeProgress\x12Z\n" +
	"\x17workspace_list_response\x18\a \x01(\v2 .laelia.v1.WorkspaceListResponseH\x00R\x15workspaceListResponse\x12Z\n" +
	"\x17workspace_read_response\x18\b \x01(\v2 .laelia.v1.WorkspaceReadResponseH\x00R\x15workspaceReadResponse\x12^\n" +
	"\x19prompt_release_notice_ack\x18\t \x01(\v2!.laelia.v1.PromptReleaseNoticeAckH\x00R\x16promptReleaseNoticeAck\x12J\n" +
	"\x11models_discovered\x18\n" +
	" \x01(\v2\x1b.laelia.v1.ModelsDiscoveredH\x00R\x10modelsDiscovered\x12]\n" +
	"\x18machine_metrics_response\x18\v \x01(\v2!.laelia.v1.MachineMetricsResponseH\x00R\x16machineMetricsResponseB\t\n" +
	"\amessage\"\xb9\t\n" +
	"\x1bManagerMachineStreamMessage\x12G\n" +
	"\x10agent_assignment\x18\x01 \x01(\v2\x1a.laelia.v1.AgentAssignmentH\x00R\x0fagentAssignment\x12;\n" +
	"\fremove_agent\x18\x02 \x01(\v2\x16.laelia.v1.RemoveAgentH\x00R\vremoveAgent\x12N\n" +
	"\x13agent_config_update\x18\x03 \x01(\v2\x1c.laelia.v1.AgentConfigUpdateH\x00R\x11agentConfigUpdate\x12M\n" +
	"\x12discover_providers\x18\x04 \x01(\v2\x1c.laelia.v1.DiscoverProvidersH\x00R\x11discoverProviders\x12%\n" +
	"\x04pong\x18\x05 \x01(\v2\x0f.laelia.v1.PongH\x00R\x04pong\x12Z\n" +
	"\x17reload_agent_assignment\x18\x06 \x01(\v2 .laelia.v1.ReloadAgentAssignmentH\x00R\x15reloadAgentAssignment\x12m\n" +
	"\x1emachine_workspace_scan_request\x18\a \x01(\v2&.laelia.v1.MachineWorkspaceScanRequestH\x00R\x1bmachineWorkspaceScanRequest\x12W\n" +
	"\x16delete_agent_workspace\x18\b \x01(\v2\x1f.laelia.v1.DeleteAgentWorkspaceH\x00R\x14deleteAgentWorkspace\x12D\n" +
	"\x0fupgrade_request\x18\t \x01(\v2\x19.laelia.v1.UpgradeRequestH\x00R\x0eupgradeRequest\x12D\n" +
	"\x0fdiscover_models\x18\v \x01(\v2\x19.laelia.v1.DiscoverModelsH\x00R\x0ediscoverModels\x12>\n" +
	"\rrestart_agent\x18\f \x01(\v2\x17.laelia.v1.RestartAgentH\x00R\frestartAgent\x12E\n" +
	"\ragent_control\x18\r \x01(\v2\x1e.laelia.v1.AgentControlRequestH\x00R\fagentControl\x12W\n" +
	"\x16workspace_list_request\x18\x0e \x01(\v2\x1f.laelia.v1.WorkspaceListRequestH\x00R\x14workspaceListRequest\x12W\n" +
	"\x16workspace_read_request\x18\x0f \x01(\v2\x1f.laelia.v1.WorkspaceReadRequestH\x00R\x14workspaceReadRequest\x12Z\n" +
	"\x17machine_metrics_request\x18\x10 \x01(\v2 .laelia.v1.MachineMetricsRequestH\x00R\x15machineMetricsRequestB\t\n" +
	"\amessage\"\xa2\x02\n" +
	"\x13AgentControlRequest\x12\x1d\n" +
	"\n" +
	"agent_name\x18\x01 \x01(\tR\tagentName\x122\n" +
	"\x06cancel\x18\x02 \x01(\v2\x18.laelia.v1.CancelMessageH\x00R\x06cancel\x12/\n" +
	"\x05steer\x18\x03 \x01(\v2\x17.laelia.v1.SteerMessageH\x00R\x05steer\x125\n" +
	"\x04wake\x18\x04 \x01(\v2\x1f.laelia.v1.NewMessagesAvailableH\x00R\x04wake\x12E\n" +
	"\rprompt_notice\x18\x05 \x01(\v2\x1e.laelia.v1.PromptReleaseNoticeH\x00R\fpromptNoticeB\t\n" +
	"\acontrol\"-\n" +
	"\fMachineReady\x12\x1d\n" +
	"\n" +
	"session_id\x18\x01 \x01(\tR\tsessionId\",\n" +
	"\vRemoveAgent\x12\x1d\n" +
	"\n" +
	"agent_name\x18\x01 \x01(\tR\tagentName\"-\n" +
	"\fRestartAgent\x12\x1d\n" +
	"\n" +
	"agent_name\x18\x01 \x01(\tR\tagentName\"5\n" +
	"\x14DeleteAgentWorkspace\x12\x1d\n" +
	"\n" +
	"agent_name\x18\x01 \x01(\tR\tagentName\"l\n" +
	"\x11AgentConfigUpdate\x12\x1d\n" +
	"\n" +
	"agent_name\x18\x01 \x01(\tR\tagentName\x128\n" +
	"\n" +
	"acp_config\x18\x02 \x01(\v2\x19.laelia.v1.AgentACPConfigR\tacpConfig\"1\n" +
	"\x17MachineDisconnectNotice\x12\x16\n" +
	"\x06reason\x18\x01 \x01(\tR\x06reason\"\xb9\x01\n" +
	"\x0eDiscoverModels\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12\x1a\n" +
	"\bprovider\x18\x02 \x01(\tR\bprovider\x124\n" +
	"\x03env\x18\x03 \x03(\v2\".laelia.v1.DiscoverModels.EnvEntryR\x03env\x1a6\n" +
	"\bEnvEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\"\x98\x01\n" +
	"\x10ModelsDiscovered\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12\x1a\n" +
	"\bprovider\x18\x02 \x01(\tR\bprovider\x123\n" +
	"\x06models\x18\x03 \x03(\v2\x1b.laelia.v1.AgentModelOptionR\x06models\x12\x14\n" +
	"\x05error\x18\x04 \x01(\tR\x05error\"r\n" +
	"\x15ReloadAgentAssignment\x12\x1d\n" +
	"\n" +
	"agent_name\x18\x01 \x01(\tR\tagentName\x12:\n" +
	"\n" +
	"assignment\x18\x02 \x01(\v2\x1a.laelia.v1.AgentAssignmentR\n" +
	"assignment\"<\n" +
	"\x1bMachineWorkspaceScanRequest\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\"\xca\x01\n" +
	"\x17MachineWorkspaceSummary\x12%\n" +
	"\x0edirectory_name\x18\x01 \x01(\tR\rdirectoryName\x12(\n" +
	"\x10total_size_bytes\x18\x02 \x01(\x03R\x0etotalSizeBytes\x12?\n" +
	"\rlast_modified\x18\x03 \x01(\v2\x1a.google.protobuf.TimestampR\flastModified\x12\x1d\n" +
	"\n" +
	"file_count\x18\x04 \x01(\x03R\tfileCount\"\x81\x01\n" +
	"\x1cMachineWorkspaceScanResponse\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12B\n" +
	"\n" +
	"workspaces\x18\x02 \x03(\v2\".laelia.v1.MachineWorkspaceSummaryR\n" +
	"workspaces\"6\n" +
	"\x15MachineMetricsRequest\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\"g\n" +
	"\x16MachineMetricsResponse\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12\x18\n" +
	"\apayload\x18\x02 \x01(\tR\apayload\x12\x14\n" +
	"\x05error\x18\x03 \x01(\tR\x05error\".\n" +
	"\x18GetMachineMetricsRequest\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\"5\n" +
	"\x19GetMachineMetricsResponse\x12\x18\n" +
	"\apayload\x18\x01 \x01(\tR\apayload*\x8f\x01\n" +
	"\x0fUploadEntryKind\x12!\n" +
	"\x1dUPLOAD_ENTRY_KIND_UNSPECIFIED\x10\x00\x12\x1e\n" +
	"\x1aUPLOAD_ENTRY_KIND_PROGRESS\x10\x01\x12\x1b\n" +
	"\x17UPLOAD_ENTRY_KIND_EVENT\x10\x02\x12\x1c\n" +
	"\x18UPLOAD_ENTRY_KIND_RESULT\x10\x03*\x86\x02\n" +
	"\x11ProvisioningPhase\x12\"\n" +
	"\x1ePROVISIONING_PHASE_UNSPECIFIED\x10\x00\x12\x1e\n" +
	"\x1aPROVISIONING_PHASE_PENDING\x10\x01\x12#\n" +
	"\x1fPROVISIONING_PHASE_PROVISIONING\x10\x02\x12\"\n" +
	"\x1ePROVISIONING_PHASE_PROVISIONED\x10\x03\x12\x1d\n" +
	"\x19PROVISIONING_PHASE_FAILED\x10\x04\x12%\n" +
	"!PROVISIONING_PHASE_DEPROVISIONING\x10\x05\x12\x1e\n" +
	"\x1aPROVISIONING_PHASE_DELETED\x10\x062\x8a\x0e\n" +
	"\x0eMachineService\x12l\n" +
	"\fListMachines\x12\x1e.laelia.v1.ListMachinesRequest\x1a\x1f.laelia.v1.ListMachinesResponse\"\x1b\x8a\xea0\x13laelia.machines.get\x90\xea0\x01\x12b\n" +
	"\n" +
	"GetMachine\x12\x1c.laelia.v1.GetMachineRequest\x1a\x12.laelia.v1.Machine\"\"\xdaA\x04name\x8a\xea0\x13laelia.machines.get\x90\xea0\x01\x12R\n" +
	"\rDeleteMachine\x12\x1f.laelia.v1.DeleteMachineRequest\x1a\x16.google.protobuf.Empty\"\b\x90\xea0\x01\x98\xea0\x01\x12N\n" +
	"\rUpdateMachine\x12\x1f.laelia.v1.UpdateMachineRequest\x1a\x12.laelia.v1.Machine\"\b\x90\xea0\x01\x98\xea0\x01\x12}\n" +
	"\x18TransferMachineOwnership\x12*.laelia.v1.TransferMachineOwnershipRequest\x1a+.laelia.v1.TransferMachineOwnershipResponse\"\b\x90\xea0\x01\x98\xea0\x01\x12k\n" +
	"\x12RevokeMachineToken\x12$.laelia.v1.RevokeMachineTokenRequest\x1a%.laelia.v1.RevokeMachineTokenResponse\"\b\x90\xea0\x01\x98\xea0\x01\x12d\n" +
	"\x16ForceDisconnectMachine\x12(.laelia.v1.ForceDisconnectMachineRequest\x1a\x16.google.protobuf.Empty\"\b\x90\xea0\x01\x98\xea0\x01\x12{\n" +
	"\x11ListMachineAgents\x12#.laelia.v1.ListMachineAgentsRequest\x1a$.laelia.v1.ListMachineAgentsResponse\"\x1b\x8a\xea0\x13laelia.machines.get\x90\xea0\x01\x12z\n" +
	"\x17RefreshMachineProviders\x12).laelia.v1.RefreshMachineProvidersRequest\x1a*.laelia.v1.RefreshMachineProvidersResponse\"\b\x90\xea0\x01\x98\xea0\x01\x12q\n" +
	"\x14RefreshMachineModels\x12&.laelia.v1.RefreshMachineModelsRequest\x1a'.laelia.v1.RefreshMachineModelsResponse\"\b\x90\xea0\x01\x98\xea0\x01\x12T\n" +
	"\x0eUpgradeMachine\x12 .laelia.v1.UpgradeMachineRequest\x1a\x16.google.protobuf.Empty\"\b\x90\xea0\x01\x98\xea0\x01\x12t\n" +
	"\x15ListMachineWorkspaces\x12'.laelia.v1.ListMachineWorkspacesRequest\x1a(.laelia.v1.ListMachineWorkspacesResponse\"\b\x90\xea0\x01\x98\xea0\x01\x12h\n" +
	"\x11GetMachineMetrics\x12#.laelia.v1.GetMachineMetricsRequest\x1a$.laelia.v1.GetMachineMetricsResponse\"\b\x90\xea0\x01\x98\xea0\x01\x12_\n" +
	"\x0eConnectMachine\x12 .laelia.v1.ConnectMachineRequest\x1a!.laelia.v1.ConnectMachineResponse\"\b\x90\xea0\x02\x98\xea0\x01\x12a\n" +
	"\x10MachineHeartbeat\x12\".laelia.v1.MachineHeartbeatRequest\x1a#.laelia.v1.MachineHeartbeatResponse\"\x04\x90\xea0\x02\x12Z\n" +
	"\x11MachineDisconnect\x12#.laelia.v1.MachineDisconnectRequest\x1a\x16.google.protobuf.Empty\"\b\x90\xea0\x02\x98\xea0\x01\x12n\n" +
	"\x13RefreshMachineToken\x12%.laelia.v1.RefreshMachineTokenRequest\x1a&.laelia.v1.RefreshMachineTokenResponse\"\b\x90\xea0\x02\x98\xea0\x012\xb2\x02\n" +
	"\x14MachineStreamService\x12]\n" +
	"\x0eMachineChannel\x12\x1f.laelia.v1.MachineStreamMessage\x1a&.laelia.v1.ManagerMachineStreamMessage(\x010\x01\x12d\n" +
	"\x11UploadCommandData\x12#.laelia.v1.UploadCommandDataRequest\x1a$.laelia.v1.UploadCommandDataResponse\"\x04\x90\xea0\x02\x12U\n" +
	"\fBeginSession\x12\x1e.laelia.v1.BeginSessionRequest\x1a\x1f.laelia.v1.BeginSessionResponse\"\x04\x90\xea0\x02B1Z/github.com/Ranxy/laelia/backend/generated-go/v1b\x06proto3"

var (
	file_v1_machine_proto_rawDescOnce sync.Once
	file_v1_machine_proto_rawDescData []byte
)

func file_v1_machine_proto_rawDescGZIP() []byte {
	file_v1_machine_proto_rawDescOnce.Do(func() {
		file_v1_machine_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_v1_machine_proto_rawDesc), len(file_v1_machine_proto_rawDesc)))
	})
	return file_v1_machine_proto_rawDescData
}

var file_v1_machine_proto_enumTypes = make([]protoimpl.EnumInfo, 3)
var file_v1_machine_proto_msgTypes = make([]protoimpl.MessageInfo, 64)
var file_v1_machine_proto_goTypes = []any{
	(UploadEntryKind)(0),                     // 0: laelia.v1.UploadEntryKind
	(ProvisioningPhase)(0),                   // 1: laelia.v1.ProvisioningPhase
	(MachineStatus_ConnectionState)(0),       // 2: laelia.v1.MachineStatus.ConnectionState
	(*BeginSessionRequest)(nil),              // 3: laelia.v1.BeginSessionRequest
	(*UploadCommandDataEntry)(nil),           // 4: laelia.v1.UploadCommandDataEntry
	(*UploadCommandDataRequest)(nil),         // 5: laelia.v1.UploadCommandDataRequest
	(*UploadCommandDataAck)(nil),             // 6: laelia.v1.UploadCommandDataAck
	(*UploadCommandDataRejection)(nil),       // 7: laelia.v1.UploadCommandDataRejection
	(*UploadCommandDataResponse)(nil),        // 8: laelia.v1.UploadCommandDataResponse
	(*UpdateMachineRequest)(nil),             // 9: laelia.v1.UpdateMachineRequest
	(*TransferMachineOwnershipRequest)(nil),  // 10: laelia.v1.TransferMachineOwnershipRequest
	(*TransferMachineOwnershipResponse)(nil), // 11: laelia.v1.TransferMachineOwnershipResponse
	(*RevokeMachineTokenRequest)(nil),        // 12: laelia.v1.RevokeMachineTokenRequest
	(*RevokeMachineTokenResponse)(nil),       // 13: laelia.v1.RevokeMachineTokenResponse
	(*ForceDisconnectMachineRequest)(nil),    // 14: laelia.v1.ForceDisconnectMachineRequest
	(*ListMachineAgentsRequest)(nil),         // 15: laelia.v1.ListMachineAgentsRequest
	(*ListMachineAgentsResponse)(nil),        // 16: laelia.v1.ListMachineAgentsResponse
	(*RefreshMachineProvidersRequest)(nil),   // 17: laelia.v1.RefreshMachineProvidersRequest
	(*RefreshMachineProvidersResponse)(nil),  // 18: laelia.v1.RefreshMachineProvidersResponse
	(*RefreshMachineModelsRequest)(nil),      // 19: laelia.v1.RefreshMachineModelsRequest
	(*RefreshMachineModelsResponse)(nil),     // 20: laelia.v1.RefreshMachineModelsResponse
	(*UpgradeMachineRequest)(nil),            // 21: laelia.v1.UpgradeMachineRequest
	(*ListMachineWorkspacesRequest)(nil),     // 22: laelia.v1.ListMachineWorkspacesRequest
	(*ListMachineWorkspacesResponse)(nil),    // 23: laelia.v1.ListMachineWorkspacesResponse
	(*ConnectMachineRequest)(nil),            // 24: laelia.v1.ConnectMachineRequest
	(*ConnectMachineResponse)(nil),           // 25: laelia.v1.ConnectMachineResponse
	(*MachineHeartbeatRequest)(nil),          // 26: laelia.v1.MachineHeartbeatRequest
	(*MachineHeartbeatResponse)(nil),         // 27: laelia.v1.MachineHeartbeatResponse
	(*MachineDisconnectRequest)(nil),         // 28: laelia.v1.MachineDisconnectRequest
	(*RefreshMachineTokenRequest)(nil),       // 29: laelia.v1.RefreshMachineTokenRequest
	(*RefreshMachineTokenResponse)(nil),      // 30: laelia.v1.RefreshMachineTokenResponse
	(*ListMachinesRequest)(nil),              // 31: laelia.v1.ListMachinesRequest
	(*ListMachinesResponse)(nil),             // 32: laelia.v1.ListMachinesResponse
	(*GetMachineRequest)(nil),                // 33: laelia.v1.GetMachineRequest
	(*DeleteMachineRequest)(nil),             // 34: laelia.v1.DeleteMachineRequest
	(*Machine)(nil),                          // 35: laelia.v1.Machine
	(*MachineSummary)(nil),                   // 36: laelia.v1.MachineSummary
	(*UpgradeRequest)(nil),                   // 37: laelia.v1.UpgradeRequest
	(*UpgradeProgress)(nil),                  // 38: laelia.v1.UpgradeProgress
	(*ProvisioningStatus)(nil),               // 39: laelia.v1.ProvisioningStatus
	(*MachineInfo)(nil),                      // 40: laelia.v1.MachineInfo
	(*MachineStatus)(nil),                    // 41: laelia.v1.MachineStatus
	(*AgentAssignment)(nil),                  // 42: laelia.v1.AgentAssignment
	(*MachineStreamMessage)(nil),             // 43: laelia.v1.MachineStreamMessage
	(*ManagerMachineStreamMessage)(nil),      // 44: laelia.v1.ManagerMachineStreamMessage
	(*AgentControlRequest)(nil),              // 45: laelia.v1.AgentControlRequest
	(*MachineReady)(nil),                     // 46: laelia.v1.MachineReady
	(*RemoveAgent)(nil),                      // 47: laelia.v1.RemoveAgent
	(*RestartAgent)(nil),                     // 48: laelia.v1.RestartAgent
	(*DeleteAgentWorkspace)(nil),             // 49: laelia.v1.DeleteAgentWorkspace
	(*AgentConfigUpdate)(nil),                // 50: laelia.v1.AgentConfigUpdate
	(*MachineDisconnectNotice)(nil),          // 51: laelia.v1.MachineDisconnectNotice
	(*DiscoverModels)(nil),                   // 52: laelia.v1.DiscoverModels
	(*ModelsDiscovered)(nil),                 // 53: laelia.v1.ModelsDiscovered
	(*ReloadAgentAssignment)(nil),            // 54: laelia.v1.ReloadAgentAssignment
	(*MachineWorkspaceScanRequest)(nil),      // 55: laelia.v1.MachineWorkspaceScanRequest
	(*MachineWorkspaceSummary)(nil),          // 56: laelia.v1.MachineWorkspaceSummary
	(*MachineWorkspaceScanResponse)(nil),     // 57: laelia.v1.MachineWorkspaceScanResponse
	(*MachineMetricsRequest)(nil),            // 58: laelia.v1.MachineMetricsRequest
	(*MachineMetricsResponse)(nil),           // 59: laelia.v1.MachineMetricsResponse
	(*GetMachineMetricsRequest)(nil),         // 60: laelia.v1.GetMachineMetricsRequest
	(*GetMachineMetricsResponse)(nil),        // 61: laelia.v1.GetMachineMetricsResponse
	nil,                                      // 62: laelia.v1.Machine.LabelsEntry
	nil,                                      // 63: laelia.v1.ProvisioningStatus.WorkloadLabelsEntry
	nil,                                      // 64: laelia.v1.ProvisioningStatus.MachineParamsEntry
	nil,                                      // 65: laelia.v1.MachineInfo.LabelsEntry
	nil,                                      // 66: laelia.v1.DiscoverModels.EnvEntry
	(*timestamppb.Timestamp)(nil),            // 67: google.protobuf.Timestamp
	(*CommandProgress)(nil),                  // 68: laelia.v1.CommandProgress
	(*CommandEvent)(nil),                     // 69: laelia.v1.CommandEvent
	(*CommandResult)(nil),                    // 70: laelia.v1.CommandResult
	(*AgentSummary)(nil),                     // 71: laelia.v1.AgentSummary
	(*AgentProviderInfo)(nil),                // 72: laelia.v1.AgentProviderInfo
	(*AgentACPConfig)(nil),                   // 73: laelia.v1.AgentACPConfig
	(*AgentModelOption)(nil),                 // 74: laelia.v1.AgentModelOption
	(State)(0),                               // 75: laelia.v1.State
	(*AgentCapability)(nil),                  // 76: laelia.v1.AgentCapability
	(*Ping)(nil),                             // 77: laelia.v1.Ping
	(*ProvidersDiscovered)(nil),              // 78: laelia.v1.ProvidersDiscovered
	(*WorkspaceListResponse)(nil),            // 79: laelia.v1.WorkspaceListResponse
	(*WorkspaceReadResponse)(nil),            // 80: laelia.v1.WorkspaceReadResponse
	(*PromptReleaseNoticeAck)(nil),           // 81: laelia.v1.PromptReleaseNoticeAck
	(*DiscoverProviders)(nil),                // 82: laelia.v1.DiscoverProviders
	(*Pong)(nil),                             // 83: laelia.v1.Pong
	(*WorkspaceListRequest)(nil),             // 84: laelia.v1.WorkspaceListRequest
	(*WorkspaceReadRequest)(nil),             // 85: laelia.v1.WorkspaceReadRequest
	(*CancelMessage)(nil),                    // 86: laelia.v1.CancelMessage
	(*SteerMessage)(nil),                     // 87: laelia.v1.SteerMessage
	(*NewMessagesAvailable)(nil),             // 88: laelia.v1.NewMessagesAvailable
	(*PromptReleaseNotice)(nil),              // 89: laelia.v1.PromptReleaseNotice
	(*emptypb.Empty)(nil),                    // 90: google.protobuf.Empty
	(*BeginSessionResponse)(nil),             // 91: laelia.v1.BeginSessionResponse
}
var file_v1_machine_proto_depIdxs = []int32{
	0,   // 0: laelia.v1.UploadCommandDataEntry.kind:type_name -> laelia.v1.UploadEntryKind
	67,  // 1: laelia.v1.UploadCommandDataEntry.agent_side_timestamp:type_name -> google.protobuf.Timestamp
	68,  // 2: laelia.v1.UploadCommandDataEntry.progress:type_name -> laelia.v1.CommandProgress
	69,  // 3: laelia.v1.UploadCommandDataEntry.event:type_name -> laelia.v1.CommandEvent
	70,  // 4: laelia.v1.UploadCommandDataEntry.result:type_name -> laelia.v1.CommandResult
	4,   // 5: laelia.v1.UploadCommandDataRequest.entries:type_name -> laelia.v1.UploadCommandDataEntry
	0,   // 6: laelia.v1.UploadCommandDataRejection.kind:type_name -> laelia.v1.UploadEntryKind
	6,   // 7: laelia.v1.UploadCommandDataResponse.acks:type_name -> laelia.v1.UploadCommandDataAck
	7,   // 8: laelia.v1.UploadCommandDataResponse.rejected:type_name -> laelia.v1.UploadCommandDataRejection
	35,  // 9: laelia.v1.TransferMachineOwnershipResponse.machine:type_name -> laelia.v1.Machine
	71,  // 10: laelia.v1.ListMachineAgentsResponse.agents:type_name -> laelia.v1.AgentSummary
	72,  // 11: laelia.v1.RefreshMachineProvidersResponse.providers:type_name -> laelia.v1.AgentProviderInfo
	73,  // 12: laelia.v1.RefreshMachineModelsRequest.acp_config:type_name -> laelia.v1.AgentACPConfig
	74,  // 13: laelia.v1.RefreshMachineModelsResponse.models:type_name -> laelia.v1.AgentModelOption
	56,  // 14: laelia.v1.ListMachineWorkspacesResponse.workspaces:type_name -> laelia.v1.MachineWorkspaceSummary
	40,  // 15: laelia.v1.ConnectMachineRequest.info:type_name -> laelia.v1.MachineInfo
	41,  // 16: laelia.v1.ConnectMachineResponse.initial_status:type_name -> laelia.v1.MachineStatus
	42,  // 17: laelia.v1.ConnectMachineResponse.assigned_agents:type_name -> laelia.v1.AgentAssignment
	67,  // 18: laelia.v1.MachineHeartbeatResponse.next_heartbeat_at:type_name -> google.protobuf.Timestamp
	67,  // 19: laelia.v1.MachineHeartbeatResponse.access_token_expires_at:type_name -> google.protobuf.Timestamp
	67,  // 20: laelia.v1.RefreshMachineTokenResponse.access_token_expires_at:type_name -> google.protobuf.Timestamp
	36,  // 21: laelia.v1.ListMachinesResponse.machines:type_name -> laelia.v1.MachineSummary
	75,  // 22: laelia.v1.Machine.state:type_name -> laelia.v1.State
	40,  // 23: laelia.v1.Machine.info:type_name -> laelia.v1.MachineInfo
	41,  // 24: laelia.v1.Machine.status:type_name -> laelia.v1.MachineStatus
	67,  // 25: laelia.v1.Machine.created_at:type_name -> google.protobuf.Timestamp
	62,  // 26: laelia.v1.Machine.labels:type_name -> laelia.v1.Machine.LabelsEntry
	38,  // 27: laelia.v1.Machine.upgrade_status:type_name -> laelia.v1.UpgradeProgress
	39,  // 28: laelia.v1.Machine.provisioning:type_name -> laelia.v1.ProvisioningStatus
	75,  // 29: laelia.v1.MachineSummary.state:type_name -> laelia.v1.State
	41,  // 30: laelia.v1.MachineSummary.status:type_name -> laelia.v1.MachineStatus
	67,  // 31: laelia.v1.MachineSummary.created_at:type_name -> google.protobuf.Timestamp
	39,  // 32: laelia.v1.MachineSummary.provisioning:type_name -> laelia.v1.ProvisioningStatus
	1,   // 33: laelia.v1.ProvisioningStatus.phase:type_name -> laelia.v1.ProvisioningPhase
	63,  // 34: laelia.v1.ProvisioningStatus.workload_labels:type_name -> laelia.v1.ProvisioningStatus.WorkloadLabelsEntry
	67,  // 35: laelia.v1.ProvisioningStatus.pending_at:type_name -> google.protobuf.Timestamp
	67,  // 36: laelia.v1.ProvisioningStatus.provisioned_at:type_name -> google.protobuf.Timestamp
	67,  // 37: laelia.v1.ProvisioningStatus.failed_at:type_name -> google.protobuf.Timestamp
	64,  // 38: laelia.v1.ProvisioningStatus.machine_params:type_name -> laelia.v1.ProvisioningStatus.MachineParamsEntry
	65,  // 39: laelia.v1.MachineInfo.labels:type_name -> laelia.v1.MachineInfo.LabelsEntry
	76,  // 40: laelia.v1.MachineInfo.capability:type_name -> laelia.v1.AgentCapability
	72,  // 41: laelia.v1.MachineInfo.available_providers:type_name -> laelia.v1.AgentProviderInfo
	2,   // 42: laelia.v1.MachineStatus.state:type_name -> laelia.v1.MachineStatus.ConnectionState
	67,  // 43: laelia.v1.MachineStatus.last_heartbeat_time:type_name -> google.protobuf.Timestamp
	67,  // 44: laelia.v1.MachineStatus.connected_time:type_name -> google.protobuf.Timestamp
	73,  // 45: laelia.v1.AgentAssignment.acp_config:type_name -> laelia.v1.AgentACPConfig
	46,  // 46: laelia.v1.MachineStreamMessage.machine_ready:type_name -> laelia.v1.MachineReady
	77,  // 47: laelia.v1.MachineStreamMessage.ping:type_name -> laelia.v1.Ping
	78,  // 48: laelia.v1.MachineStreamMessage.providers_discovered:type_name -> laelia.v1.ProvidersDiscovered
	51,  // 49: laelia.v1.MachineStreamMessage.disconnect_notice:type_name -> laelia.v1.MachineDisconnectNotice
	57,  // 50: laelia.v1.MachineStreamMessage.machine_workspace_scan_response:type_name -> laelia.v1.MachineWorkspaceScanResponse
	38,  // 51: laelia.v1.MachineStreamMessage.upgrade_progress:type_name -> laelia.v1.UpgradeProgress
	79,  // 52: laelia.v1.MachineStreamMessage.workspace_list_response:type_name -> laelia.v1.WorkspaceListResponse
	80,  // 53: laelia.v1.MachineStreamMessage.workspace_read_response:type_name -> laelia.v1.WorkspaceReadResponse
	81,  // 54: laelia.v1.MachineStreamMessage.prompt_release_notice_ack:type_name -> laelia.v1.PromptReleaseNoticeAck
	53,  // 55: laelia.v1.MachineStreamMessage.models_discovered:type_name -> laelia.v1.ModelsDiscovered
	59,  // 56: laelia.v1.MachineStreamMessage.machine_metrics_response:type_name -> laelia.v1.MachineMetricsResponse
	42,  // 57: laelia.v1.ManagerMachineStreamMessage.agent_assignment:type_name -> laelia.v1.AgentAssignment
	47,  // 58: laelia.v1.ManagerMachineStreamMessage.remove_agent:type_name -> laelia.v1.RemoveAgent
	50,  // 59: laelia.v1.ManagerMachineStreamMessage.agent_config_update:type_name -> laelia.v1.AgentConfigUpdate
	82,  // 60: laelia.v1.ManagerMachineStreamMessage.discover_providers:type_name -> laelia.v1.DiscoverProviders
	83,  // 61: laelia.v1.ManagerMachineStreamMessage.pong:type_name -> laelia.v1.Pong
	54,  // 62: laelia.v1.ManagerMachineStreamMessage.reload_agent_assignment:type_name -> laelia.v1.ReloadAgentAssignment
	55,  // 63: laelia.v1.ManagerMachineStreamMessage.machine_workspace_scan_request:type_name -> laelia.v1.MachineWorkspaceScanRequest
	49,  // 64: laelia.v1.ManagerMachineStreamMessage.delete_agent_workspace:type_name -> laelia.v1.DeleteAgentWorkspace
	37,  // 65: laelia.v1.ManagerMachineStreamMessage.upgrade_request:type_name -> laelia.v1.UpgradeRequest
	52,  // 66: laelia.v1.ManagerMachineStreamMessage.discover_models:type_name -> laelia.v1.DiscoverModels
	48,  // 67: laelia.v1.ManagerMachineStreamMessage.restart_agent:type_name -> laelia.v1.RestartAgent
	45,  // 68: laelia.v1.ManagerMachineStreamMessage.agent_control:type_name -> laelia.v1.AgentControlRequest
	84,  // 69: laelia.v1.ManagerMachineStreamMessage.workspace_list_request:type_name -> laelia.v1.WorkspaceListRequest
	85,  // 70: laelia.v1.ManagerMachineStreamMessage.workspace_read_request:type_name -> laelia.v1.WorkspaceReadRequest
	58,  // 71: laelia.v1.ManagerMachineStreamMessage.machine_metrics_request:type_name -> laelia.v1.MachineMetricsRequest
	86,  // 72: laelia.v1.AgentControlRequest.cancel:type_name -> laelia.v1.CancelMessage
	87,  // 73: laelia.v1.AgentControlRequest.steer:type_name -> laelia.v1.SteerMessage
	88,  // 74: laelia.v1.AgentControlRequest.wake:type_name -> laelia.v1.NewMessagesAvailable
	89,  // 75: laelia.v1.AgentControlRequest.prompt_notice:type_name -> laelia.v1.PromptReleaseNotice
	73,  // 76: laelia.v1.AgentConfigUpdate.acp_config:type_name -> laelia.v1.AgentACPConfig
	66,  // 77: laelia.v1.DiscoverModels.env:type_name -> laelia.v1.DiscoverModels.EnvEntry
	74,  // 78: laelia.v1.ModelsDiscovered.models:type_name -> laelia.v1.AgentModelOption
	42,  // 79: laelia.v1.ReloadAgentAssignment.assignment:type_name -> laelia.v1.AgentAssignment
	67,  // 80: laelia.v1.MachineWorkspaceSummary.last_modified:type_name -> google.protobuf.Timestamp
	56,  // 81: laelia.v1.MachineWorkspaceScanResponse.workspaces:type_name -> laelia.v1.MachineWorkspaceSummary
	31,  // 82: laelia.v1.MachineService.ListMachines:input_type -> laelia.v1.ListMachinesRequest
	33,  // 83: laelia.v1.MachineService.GetMachine:input_type -> laelia.v1.GetMachineRequest
	34,  // 84: laelia.v1.MachineService.DeleteMachine:input_type -> laelia.v1.DeleteMachineRequest
	9,   // 85: laelia.v1.MachineService.UpdateMachine:input_type -> laelia.v1.UpdateMachineRequest
	10,  // 86: laelia.v1.MachineService.TransferMachineOwnership:input_type -> laelia.v1.TransferMachineOwnershipRequest
	12,  // 87: laelia.v1.MachineService.RevokeMachineToken:input_type -> laelia.v1.RevokeMachineTokenRequest
	14,  // 88: laelia.v1.MachineService.ForceDisconnectMachine:input_type -> laelia.v1.ForceDisconnectMachineRequest
	15,  // 89: laelia.v1.MachineService.ListMachineAgents:input_type -> laelia.v1.ListMachineAgentsRequest
	17,  // 90: laelia.v1.MachineService.RefreshMachineProviders:input_type -> laelia.v1.RefreshMachineProvidersRequest
	19,  // 91: laelia.v1.MachineService.RefreshMachineModels:input_type -> laelia.v1.RefreshMachineModelsRequest
	21,  // 92: laelia.v1.MachineService.UpgradeMachine:input_type -> laelia.v1.UpgradeMachineRequest
	22,  // 93: laelia.v1.MachineService.ListMachineWorkspaces:input_type -> laelia.v1.ListMachineWorkspacesRequest
	60,  // 94: laelia.v1.MachineService.GetMachineMetrics:input_type -> laelia.v1.GetMachineMetricsRequest
	24,  // 95: laelia.v1.MachineService.ConnectMachine:input_type -> laelia.v1.ConnectMachineRequest
	26,  // 96: laelia.v1.MachineService.MachineHeartbeat:input_type -> laelia.v1.MachineHeartbeatRequest
	28,  // 97: laelia.v1.MachineService.MachineDisconnect:input_type -> laelia.v1.MachineDisconnectRequest
	29,  // 98: laelia.v1.MachineService.RefreshMachineToken:input_type -> laelia.v1.RefreshMachineTokenRequest
	43,  // 99: laelia.v1.MachineStreamService.MachineChannel:input_type -> laelia.v1.MachineStreamMessage
	5,   // 100: laelia.v1.MachineStreamService.UploadCommandData:input_type -> laelia.v1.UploadCommandDataRequest
	3,   // 101: laelia.v1.MachineStreamService.BeginSession:input_type -> laelia.v1.BeginSessionRequest
	32,  // 102: laelia.v1.MachineService.ListMachines:output_type -> laelia.v1.ListMachinesResponse
	35,  // 103: laelia.v1.MachineService.GetMachine:output_type -> laelia.v1.Machine
	90,  // 104: laelia.v1.MachineService.DeleteMachine:output_type -> google.protobuf.Empty
	35,  // 105: laelia.v1.MachineService.UpdateMachine:output_type -> laelia.v1.Machine
	11,  // 106: laelia.v1.MachineService.TransferMachineOwnership:output_type -> laelia.v1.TransferMachineOwnershipResponse
	13,  // 107: laelia.v1.MachineService.RevokeMachineToken:output_type -> laelia.v1.RevokeMachineTokenResponse
	90,  // 108: laelia.v1.MachineService.ForceDisconnectMachine:output_type -> google.protobuf.Empty
	16,  // 109: laelia.v1.MachineService.ListMachineAgents:output_type -> laelia.v1.ListMachineAgentsResponse
	18,  // 110: laelia.v1.MachineService.RefreshMachineProviders:output_type -> laelia.v1.RefreshMachineProvidersResponse
	20,  // 111: laelia.v1.MachineService.RefreshMachineModels:output_type -> laelia.v1.RefreshMachineModelsResponse
	90,  // 112: laelia.v1.MachineService.UpgradeMachine:output_type -> google.protobuf.Empty
	23,  // 113: laelia.v1.MachineService.ListMachineWorkspaces:output_type -> laelia.v1.ListMachineWorkspacesResponse
	61,  // 114: laelia.v1.MachineService.GetMachineMetrics:output_type -> laelia.v1.GetMachineMetricsResponse
	25,  // 115: laelia.v1.MachineService.ConnectMachine:output_type -> laelia.v1.ConnectMachineResponse
	27,  // 116: laelia.v1.MachineService.MachineHeartbeat:output_type -> laelia.v1.MachineHeartbeatResponse
	90,  // 117: laelia.v1.MachineService.MachineDisconnect:output_type -> google.protobuf.Empty
	30,  // 118: laelia.v1.MachineService.RefreshMachineToken:output_type -> laelia.v1.RefreshMachineTokenResponse
	44,  // 119: laelia.v1.MachineStreamService.MachineChannel:output_type -> laelia.v1.ManagerMachineStreamMessage
	8,   // 120: laelia.v1.MachineStreamService.UploadCommandData:output_type -> laelia.v1.UploadCommandDataResponse
	91,  // 121: laelia.v1.MachineStreamService.BeginSession:output_type -> laelia.v1.BeginSessionResponse
	102, // [102:122] is the sub-list for method output_type
	82,  // [82:102] is the sub-list for method input_type
	82,  // [82:82] is the sub-list for extension type_name
	82,  // [82:82] is the sub-list for extension extendee
	0,   // [0:82] is the sub-list for field type_name
}

func init() { file_v1_machine_proto_init() }
func file_v1_machine_proto_init() {
	if File_v1_machine_proto != nil {
		return
	}
	file_v1_agent_proto_init()
	file_v1_annotation_proto_init()
	file_v1_command_proto_init()
	file_v1_common_proto_init()
	file_v1_machine_proto_msgTypes[1].OneofWrappers = []any{
		(*UploadCommandDataEntry_Progress)(nil),
		(*UploadCommandDataEntry_Event)(nil),
		(*UploadCommandDataEntry_Result)(nil),
	}
	file_v1_machine_proto_msgTypes[40].OneofWrappers = []any{
		(*MachineStreamMessage_MachineReady)(nil),
		(*MachineStreamMessage_Ping)(nil),
		(*MachineStreamMessage_ProvidersDiscovered)(nil),
		(*MachineStreamMessage_DisconnectNotice)(nil),
		(*MachineStreamMessage_MachineWorkspaceScanResponse)(nil),
		(*MachineStreamMessage_UpgradeProgress)(nil),
		(*MachineStreamMessage_WorkspaceListResponse)(nil),
		(*MachineStreamMessage_WorkspaceReadResponse)(nil),
		(*MachineStreamMessage_PromptReleaseNoticeAck)(nil),
		(*MachineStreamMessage_ModelsDiscovered)(nil),
		(*MachineStreamMessage_MachineMetricsResponse)(nil),
	}
	file_v1_machine_proto_msgTypes[41].OneofWrappers = []any{
		(*ManagerMachineStreamMessage_AgentAssignment)(nil),
		(*ManagerMachineStreamMessage_RemoveAgent)(nil),
		(*ManagerMachineStreamMessage_AgentConfigUpdate)(nil),
		(*ManagerMachineStreamMessage_DiscoverProviders)(nil),
		(*ManagerMachineStreamMessage_Pong)(nil),
		(*ManagerMachineStreamMessage_ReloadAgentAssignment)(nil),
		(*ManagerMachineStreamMessage_MachineWorkspaceScanRequest)(nil),
		(*ManagerMachineStreamMessage_DeleteAgentWorkspace)(nil),
		(*ManagerMachineStreamMessage_UpgradeRequest)(nil),
		(*ManagerMachineStreamMessage_DiscoverModels)(nil),
		(*ManagerMachineStreamMessage_RestartAgent)(nil),
		(*ManagerMachineStreamMessage_AgentControl)(nil),
		(*ManagerMachineStreamMessage_WorkspaceListRequest)(nil),
		(*ManagerMachineStreamMessage_WorkspaceReadRequest)(nil),
		(*ManagerMachineStreamMessage_MachineMetricsRequest)(nil),
	}
	file_v1_machine_proto_msgTypes[42].OneofWrappers = []any{
		(*AgentControlRequest_Cancel)(nil),
		(*AgentControlRequest_Steer)(nil),
		(*AgentControlRequest_Wake)(nil),
		(*AgentControlRequest_PromptNotice)(nil),
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_v1_machine_proto_rawDesc), len(file_v1_machine_proto_rawDesc)),
			NumEnums:      3,
			NumMessages:   64,
			NumExtensions: 0,
			NumServices:   2,
		},
		GoTypes:           file_v1_machine_proto_goTypes,
		DependencyIndexes: file_v1_machine_proto_depIdxs,
		EnumInfos:         file_v1_machine_proto_enumTypes,
		MessageInfos:      file_v1_machine_proto_msgTypes,
	}.Build()
	File_v1_machine_proto = out.File
	file_v1_machine_proto_goTypes = nil
	file_v1_machine_proto_depIdxs = nil
}
