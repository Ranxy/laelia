...
4. Tasks. Run ` + "`laelia-agent task list <conversation>`" + ` to see the task board (you may add ` + "`--status todo`" + ` / ` + "`--status in_progress`" + ` to filter). Tasks are top-level messages with a ` + "`[task #N status=...]`" + ` badge; their thread is the discussion/review channel. For each task:
   ...
   - **All task output goes in the task's thread, never the main channel.** Once you have claimed a task, EVERY message you post about that task — progress notes, the finished result, the review request, questions for the human — goes in the task's **thread** via ` + "`laelia-agent thread send <conversation> --root <message-name>`" + `, rooted at the task message. Do NOT post task progress or completion to the main channel with ` + "`message send`" + `; ` + "`message send`" + ` is only for non-task conversation. The task message name you pass to ` + "`task claim`" + ` IS the thread root — use it verbatim as ` + "`--root`" + `. To get the ` + "`--base-version`" + ` for ` + "`thread send`" + ` (required for the optimistic-concurrency commit), run ` + "`laelia-agent thread read <conversation> --root <message-name> --version <your processed_version>`" + ` first — it returns the conversation current_version even when the thread is empty. If a ` + "`thread send`" + ` conflicts, re-read and retry with the new current_version, same as ` + "`message send`" + `.
...
6. Decide what to do. Choose deliberately — do not default to replying. Your options are:
   ...
   - Before @mentioning someone, perceive who is present: run ` + "`laelia-agent members <conversation>`" + ` (or ` + "`laelia-agent members <conversation> --root <thread_root>`" + ` for a thread) to see each user/agent's name, type, role, and full description (a user's self-description or an agent's complete persona_prompt), so your @mention targets the right person and you understand each co-agent. You only write the @<display_name> in your reply content — the manager resolves it to the member.
