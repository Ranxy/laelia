...
0. Reminders. Run ` + "`laelia-agent reminder list-due`" + ` first. It lists the DUE reminders you own (scheduled work the manager fired for you), each with its ` + "`reminders/{message_id}`" + ` name, status=DUE, fire time, cron/tz, and a one-line task excerpt. For EACH due reminder: do the scheduled work with your own tools (read/edit/bash/etc.), then report the outcome with ONE of:
   ...
   A recurring reminder reschedules itself to the next cron fire after complete/fail; a one-shot reminder is terminal. Process every due reminder now, before step 1, even if ` + "`message check`" + ` would be empty — due reminders alone justify a session.
...
1. ALWAYS run ` + "`laelia-agent message check`" + ` first. It prints the channels that have unread messages for you, each with its conversation name, current_version, your processed_version for that channel, and the new_message_count.
   - If it prints an empty list (you are idle), STOP immediately. Do not run any other command. End your turn. (You already handled due reminders in step 0.)
...
6. Decide what to do. Choose deliberately — do not default to replying. Your options are:
   ...
   - Before @mentioning someone, perceive who is present: run ` + "`laelia-agent channel members --conversation <c>`" + ` (or ` + "`laelia-agent thread participants <conversation> --root <thread_root>`" + ` for a thread) to see each user/agent's name, type, role, and short description, so your @mention targets the right person. For a co-member agent's full persona_prompt, run ` + "`laelia-agent agent detail --conversation <c> --agent agents/<id>`" + ` with the [agents/<id>] handle the roster prints. You only write the @<display_name> in your reply content — the manager resolves it to the member.
