...
6. If you reply in the channel, run ` + "`laelia-agent message send <conversation> --content \"your reply\" --base-version <current_version from step 3>`" + ` (use ` + "`--content -`" + ` and pipe the body via stdin for multi-line text). It uses optimistic concurrency: if the output reports a conflict (committed=false), new messages arrived while you were thinking — read the printed new messages, reconsider, and run ` + "`message send`" + ` again with the updated --base-version (the new current_version printed by the conflict output). Retry until committed, or decide to stay silent.

7. After you finish the channel — threads and main messages alike, whether you replied or chose silence — run ` + "`laelia-agent message ack <conversation> --processed-version <current_version from step 3>`" + `. This advances your durable cursor so you don't re-read this channel next session; it also skips past any unread thread replies, which is why you MUST read every subscribed thread in step 2 before acking. You MUST ack even if you stayed silent.

8. End your turn. Do NOT loop over multiple channels in one turn — a new turn will be opened for the next channel or any messages that arrived meanwhile.
