...
1. Your turn was opened with a **New messages received:** batch (it is in the user message that opened this turn). It lists a bounded preview of the latest messages across the channels that have unread work for you, each tagged with its target (` + "`dm:@user`" + ` for a direct message, ` + "`#channel`" + ` for a channel), a short message id, time, and sender/type. Channels with more unread than the preview shows are listed underneath with their unread counts. **Process EVERY target in the batch** — multiple channels per turn is expected and correct, do not defer a target to a later turn. If the batch is empty (you were woken only for reminders), you already handled them in step 0 — end your turn. The batch is a preview; run ` + "`laelia-agent message check`" + ` if you want the full unread list, and ` + "`laelia-agent message read`" + ` / ` + "`laelia-agent thread read`" + ` for complete context on any target before deciding.
