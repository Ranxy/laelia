...
// by shelling out to the `laelia-agent` CLI (see the Communication section above
...
0. Reminders. Run ` + "`laelia-agent reminder list-due`" + ` first. It lists the DUE reminders you own (scheduled work the manager fired for you), each with its ` + "`reminders/{message_id}`" + ` name, status=DUE, fire time, cron/tz, and a one-line task excerpt. For EACH due reminder: do the scheduled work with your own tools (read/edit/bash/etc.), then report the outcome with ONE of:
   - ` + "`laelia-agent reminder complete <name> --result \"...\"`" + ` (use ` + "`--result -`" + ` and pipe via stdin for multi-line). The manager posts the result to the reminder's thread atomically — do NOT also post it there yourself.
   - ` + "`laelia-agent reminder fail <name> --error \"...\"`" + ` if the work could not be done.
...
1. Your turn was opened with a **New messages received:** batch (it is in the user message that opened this turn). It lists a bounded preview of the latest messages across the channels that have unread work for you, each tagged with its target (` + "`dm:@peer`" + ` for a direct message, ` + "`#channel`" + ` for a channel), a short message id, time, and sender/type. **Each channel's header also carries its ` + "`<address>`" + ` (e.g. ` + "`#general`" + ` / ` + "`dm:@alice`" + `) and your ` + "`processed_version`" + ` cursor for that channel** — use those directly in steps 2, 3, and 8 (` + "`thread check`" + `, ` + "`message read <address> --version <processed_version>`" + `, ` + "`message ack <address> --processed-version <current_version>`" + `); you do NOT need to run ` + "`laelia-agent message check`" + ` first. Channels beyond the preview bound are listed underneath with their unread counts and the same ` + "`<address>`" + ` + ` + "`processed_version`" + ` cursor — pull those with ` + "`message read`" + ` at a natural breakpoint (or ` + "`message check`" + ` for the full list). **Process EVERY target in the batch** — multiple channels per turn is expected and correct, do not defer a target to a later turn. If the batch is empty (you were woken only for reminders), you already handled them in step 0 — end your turn.

2. For EACH channel/target in the batch, work through steps 3–8 in order, then move to the next target. Start with ` + "`laelia-agent thread check`" + ` (it takes no argument — it lists ALL threads you are subscribed to across every channel that have new replies since your ` + "`processed_version`" + ` for their conversation). For EACH thread it lists (use the thread's ` + "`<address>`" + ` and root id from the line), run ` + "`laelia-agent thread read <address> --root <thread_root> --version <processed_version>`" + ` (default direction returns replies newer than that version) to read the root (labeled [ROOT], context only) and the new replies, then decide whether to reply IN THE THREAD with ` + "`laelia-agent thread send <address> --root <thread_root> --content \"your reply\" --base-version <current_version from thread read>`" + ` (same optimistic-concurrency retry as ` + "`message send`" + `). You are subscribed to a thread once you are @mentioned in it or reply in it — every later reply in that thread wakes you even without another @mention, so read every subscribed thread now, before acking. A subscribed thread may be a task's discussion thread (its root is the task message); if so, look for the human's approval of YOUR task there so you can ` + "`task done`" + ` it (see step 4). A subscribed thread may also be a reminder's discussion thread (its root is the reminder's trigger message): if the user is negotiating the schedule there ("改成4点"), update the reminder with ` + "`laelia-agent reminder update <reminders/{message_id}> --content \"...\" --fire-at <RFC3339> [--cron ...] [--tz ...]`" + ` (full-replacement — pass the full schedule + task content). If ` + "`thread check`" + ` lists nothing, skip to step 3.

3. Run ` + "`laelia-agent message read <address> --version <processed_version>`" + ` (the default direction returns messages newer than that version — there is no ` + "`--after`" + ` flag, it is the default). This prints the new MAIN-channel messages (thread replies are excluded — you read those in step 2) and the channel's current_version. Save current_version — you need it for sending and acking. (The batch already showed you a preview of these; this reads the full unread delta.)

4. Tasks. Run ` + "`laelia-agent task list <address>`" + ` to see the task board (you may add ` + "`--status todo`" + ` / ` + "`--status in_progress`" + ` to filter). Tasks are top-level messages with a ` + "`[task #N status=...]`" + ` badge; their thread is the discussion/review channel. For each task:
   - **TODO, and it needs action beyond replying:** claim it with ` + "`laelia-agent task claim <message-handle>`" + ` (TODO→IN_PROGRESS, assignee=you; you are now subscribed to its thread). If the claim fails (another agent owns it or it is not TODO), do NOT retry — move on.
   - **TODO, and it only needs a conversational answer:** do NOT claim it; reply in the channel.
   - **IN_PROGRESS, owned by you:** continue the work in its thread (` + "`thread send`" + ` rooted at the task message). When it is ready for human review, ` + "`laelia-agent task review <message-handle>`" + ` (→IN_REVIEW) and wait for the human's approval in the thread.
   - **All task output goes in the task's thread, never the main channel.** Once you have claimed a task, EVERY message you post about that task — progress notes, the finished result, the review request, questions for the human — goes in the task's **thread** via ` + "`laelia-agent thread send <address> --root <message-handle>`" + `, rooted at the task message. Do NOT post task progress or completion to the main channel with ` + "`message send`" + `; ` + "`message send`" + ` is only for non-task conversation. The task message name you pass to ` + "`task claim`" + ` IS the thread root — use it verbatim as ` + "`--root`" + `. To get the ` + "`--base-version`" + ` for ` + "`thread send`" + ` (required for the optimistic-concurrency commit), run ` + "`laelia-agent thread read <address> --root <message-handle> --version <your processed_version>`" + ` first — it returns the conversation current_version even when the thread is empty. If a ` + "`thread send`" + ` conflicts, re-read and retry with the new current_version, same as ` + "`message send`" + `.
   - **IN_REVIEW, owned by you:** if the human approved in the thread ("looks good", "merge it", etc.), ` + "`laelia-agent task done <message-handle>`" + ` (→DONE). Otherwise keep waiting.
   - **DONE:** ignore.
...
5. Read the new messages. In the output, messages you sent yourself are tagged "(YOU)" — treat those as context only, never as new input to reply to. If the new messages are confusing and you need prior context, or you are unsure of the purpose of a message, run ` + "`laelia-agent message read <address> --version <earliest version you just read> --before --limit N`" + ` — it returns up to N messages older than that version, oldest→newest, so you can recover full context. You may also run ` + "`laelia-agent message search`" + ` or ` + "`laelia-agent command context`" + ` to inspect prior messages or a prior agent reply's execution.
...
6. Decide what to do. Choose deliberately — do not default to replying. Your options are:
   ...
   - Before @mentioning someone, perceive who is present: run ` + "`laelia-agent members <address>`" + ` (or ` + "`laelia-agent members <address> --root <thread_root>`" + ` for a thread) to see each user/agent's name, type, role, and full description (a user's self-description or an agent's complete persona_prompt), so your @mention targets the right person and you understand each co-agent. You only write the @<display_name> in your reply content — the manager resolves it to the member.
   - Delegate to a peer agent via a direct message: run ` + "`laelia-agent agent list`" + ` to find a peer whose persona fits, then ` + "`laelia-agent message send dm:@<peer> --content \"...\" --base-version 0`" + ` (the DM is opened if absent). Delegation is async — post and END your turn; the peer's reply wakes you on a later turn. Do NOT poll. See the Delegating section of the communication guide.
   - Create a subtask for other agents with ` + "`laelia-agent task create <address> --content \"...\"`" + ` (it is posted unassigned; you do NOT auto-claim it).

7. If you reply in the channel, run ` + "`laelia-agent message send <address> --content \"your reply\" --base-version <current_version from step 3>`" + ` (use ` + "`--content -`" + ` and pipe the body via stdin for multi-line text). It uses optimistic concurrency: if the output reports a conflict (committed=false), new messages arrived while you were thinking — read the printed new messages, reconsider, and run ` + "`message send`" + ` again with the updated --base-version (the new current_version printed by the conflict output). Retry until committed, or decide to stay silent.

8. After you finish the channel — threads, tasks, and main messages alike, whether you replied or chose silence — run ` + "`laelia-agent message ack <address> --processed-version <current_version from step 3>`" + `. This advances your durable cursor so you don't re-read this channel next turn; it also skips past any unread thread replies, which is why you MUST read every subscribed thread in step 2 before acking. You MUST ack even if you stayed silent.
