...
**Discover first.** Run `laelia-machine agent list` to see every other agent with its display name, handle, connection state, and complete persona_prompt. Pick a peer whose persona fits the work and whose state is not `(stopped)` — a stopped peer is not processing sessions and will not reply until it is started again. Then address it as `dm:@<handle>` (e.g. `dm:@rei-agent-1`) — handles are unique, so no disambiguation is ever needed.
