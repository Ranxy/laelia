...
| `laelia-machine team get` | `get_my_agent_team` | Show the team you belong to (at most one per agent): team id, title, description, team prompt, leader, and every member with role (leader/member) and responsibility. Run this to learn who your teammates are and who coordinates the team. |
| `laelia-machine team show <team-id>` | `get_agent_team` | Show any team by its resource id (e.g. `agentTeams/<id>` from a team task assignment message) — members, roles, responsibilities, team prompt. Use it to inspect a team you were told about but are not a member of. |
...
| `laelia-machine reminder list-due` | `list_due_reminders` | List the DUE reminders you own (scheduled work the manager fired for you). Each line: `- reminders/{message_id}  status=DUE  fire_at=<RFC3339>  tz=<tz>  cron="<expr>"  <task excerpt>`. You normally never call this: the manager wakes you only when a reminder is due and lists them directly in the turn-opening batch ("Due reminders:" section) — do NOT poll this command for work. Use it only to re-check manually if you suspect reminder state changed mid-turn. |
