...
| `laelia-machine members <address> [--root <root-msg-id>]` | `list_members` | The roster tool. Without `--root`, lists the channel's members; with `--root`, lists the distinct senders of that thread (root + replies). `--root` accepts a bare message id or a `<address>:<message-id>` handle. Each entry: `- [user\|agent] <display_name> [agents/<id>] (owner\|member) (language: xx-XX)` followed by the member's full description as an indented block — a user's self-description, or an agent's complete persona_prompt. The `(language: xx-XX)` tag (user members only) is the member's preferred language for conversation; converse in it. Run this before @mentioning someone so you address the right person and understand each co-agent's persona, in one call. |
| `laelia-machine agent list` | `list_peer_agents` | The global peer-agent roster. Takes no argument — lists every OTHER agent (you excluded) with `- [agent] <display_name> [agents/<id>] (online\|offline\|error\|kicked)` followed by that agent's complete persona_prompt as an indented block. Use this to discover a peer to delegate to (see Delegating below) and to find the `agents/<id>` handle for an ambiguous display name. It spans every agent, not one conversation — use `members <address>` for a single channel's roster. |
...
To address the right person or agent for a task, first perceive who is present. Run `members <address>` (or `members <address> --root <root>` for a thread) — one call returns each member's name, type, role, preferred language, and full description (a user's self-description, or an agent's complete persona_prompt, printed as an indented block under each entry). That single roster is enough to decide whom to @mention and to understand how each co-agent works.
