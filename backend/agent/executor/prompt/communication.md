...
| `laelia-agent channel members --conversation C` | `list_channel_members` | List the users and agents in a channel. Each line: `- [user\|agent] <display_name> [agents/<id>] (owner\|member) — <short description>`. Agents carry an `[agents/<id>]` handle to pass to `agent detail`. The description is a user's self-description or an agent's persona_prompt preview (truncated). Run this before @mentioning someone so you address the right person. |
| `laelia-agent thread participants <conversation> --root <root-msg-id>` | `list_thread_participants` | List the distinct senders (users and agents) that posted in a thread (root + replies). Same line shape as `channel members`, but roles are omitted (not meaningful for threads). `--root` accepts a bare id or the full `conversations/<c>/messages/<m>` name. |
| `laelia-agent agent detail --conversation C --agent agents/<id>` | `get_agent_profile` | Fetch a co-member agent's full profile — title, status, and the complete persona_prompt (the admin-authored self-awareness prompt). Use the `[agents/<id>]` handle from `channel members`/`thread participants`. Empty persona_prompt prints `(none)`. |
...
To address the right person or agent for a task, first perceive who is present. Run `channel members --conversation <c>` (or `thread participants <conversation> --root <root>` for a thread) to see each member's name, type, role, and a short description — a user's self-description or an agent's persona_prompt preview. Each agent line prints an `[agents/<id>]` handle; pass it to `agent detail --conversation <c> --agent agents/<id>` for that agent's full persona_prompt when you need to know how it works best.
...
Match the display name the roster printed. The manager resolves case-insensitively and skips ambiguous or unknown names, so a typo just means no one is woken — verify the name against `channel members`/`thread participants` output.
