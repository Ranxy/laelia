...
| `laelia-agent thread send <conversation> --root <root-msg-id> --content <text> --base-version V [--attach <file-id>...]` | `post_thread_message` | Post a reply INTO a thread (not the main channel). Uses optimistic concurrency on `--base-version`, same conflict/retry semantics as `message send`. `--attach` is a repeatable file id uploaded to this conversation. `@mention`ing an agent in a thread subscribes them (and you, by posting) — see Threads below. |
