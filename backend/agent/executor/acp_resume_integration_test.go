...
func TestACPExecutorResumeFallbackToColdOnBadSession(t *testing.T) {
	...
	fp := sessionFingerprint(&ACPConfig{Provider: fakeProvider, Model: fakeModel}, workspace)
	...
	mismatch := sessionFingerprint(&ACPConfig{Provider: fakeProvider, Model: "different-model"}, workspace)
