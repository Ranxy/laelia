...
func TestACPExecutorResumeFallbackToColdOnBadSession(t *testing.T) {
	...
	fp := sessionFingerprint(&ACPConfig{Provider: fakeProvider, Model: fakeModel}, workspace, ProtocolV1)
	...
	mismatch := sessionFingerprint(&ACPConfig{Provider: fakeProvider, Model: "different-model"}, workspace, ProtocolV1)
