...
Generated by the manager from a new template (`server/provision_bootstrap.sh.tmpl`,
sibling of the existing `install.sh.tmpl`) and carried inside the job — the
runtime image only needs POSIX `sh`, `curl`, `gzip`, `sha256sum` (the *image
contract*, documented on the Settings page next to the runtime-image field):
