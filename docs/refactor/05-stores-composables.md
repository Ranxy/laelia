...
- ✅ 批 12(**useResourceList 退役,05 章真正终局**,`bce5a54`):最后一个消费者 command-list 迁 `composables/use-command-list.ts`(one query per (agent,status,pageToken)、scoped placeholderData 翻页保持上一页、post-send invalidate);CommandSlice 的 `commands`/`commandsLoading` 死缓存(零读者)与 `listCommands` action 一并退役,cancelCommand/steerCommand 不再 patch 死列表;`lib/use-resource-list.ts` + 测试删除。ADR-1 拍板的"列表特化薄壳"路线就此收官:全部列表域已直奔 Query。
